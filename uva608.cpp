...
string common(string s1,string s2)
{
	...
	set_intersection(s1.begin(),s1.end(),s2.begin(),s2.end(),back_inserter(ret));
...
string diff(string s1,string s2)
{
	...
	sort(s2.begin(),s2.end());
	string ret;
	set_difference(s1.begin(),s1.end(),s2.begin(),s2.end(),back_inserter(ret));
	return ret;
}
...
int main()
{
	...
	while(n--){
		...
		for(int i=1;i<ups.size();i++)
			upCom = common(upCom,ups[i]);

		for(int j=1;j<downs.size();j++)
			downCom = common(downCom,downs[j]);


		downCom = diff(downCom,allEvens);
		upCom = diff(upCom,allEvens);

		if(downCom.size())cout<<downCom<<" is the counterfeit coin and it is heavy."<<endl;
		else cout<<upCom<<" is the counterfeit coin and it is light."<<endl;
