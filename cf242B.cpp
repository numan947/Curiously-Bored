...
the following lines in main function.*/
	...
	FOR(i,0,n){
		...
		if(abs(segment[i].fi-segment[i].se)>mx){
			mx = abs(segment[i].fi-segment[i].se);
			mxIdx = i;
		}
	}

	pair<ll,ll>cover = segment[mxIdx];

	FOR(i,0,n){
		if(i==mxIdx)
			continue;
		
		if(cover.fi<=segment[i].fi&&segment[i].fi<=cover.se&&
		   cover.fi<=segment[i].se&&segment[i].se<=cover.se){
			  //ok 
		   }
		else{
			cout<<"-1"<<endl;
