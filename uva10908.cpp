...
int findMaxGrid(int r, int c,int maxR,int maxC){
	int maxAns = 1;
	int minR = min(r,maxR-r);
	int minC = min(c,maxC-c);
	int iterCount = min(minR,minC);

	char ch = grid[r][c];

	for(int i=1;i<=iterCount;i++){
		bool ok=true;
		for(int rs=r-i;ok&&rs<=r+i;rs++)
			for(int cs = c-i;ok&&cs<=c+i;cs++)
				ok=grid[rs][cs]==ch;
...
the following lines in main function.*/
	...
	while(T--){
		...
		while(Q--){
			cin>>r>>c;
			cout<<findMaxGrid(r,c,M-1,N-1)<<endl;
		}
	}
