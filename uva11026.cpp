...
ll fitness(int idx, int grpSize)
{
	if(grpSize==0)
		return 1;
	if(idx==N)
	...
	if(dp[idx][grpSize]!=-1)
		return dp[idx][grpSize];
	
	ll totalfitness = ((ara[idx]*fitness(idx+1,grpSize-1))%MOD + fitness(idx+1,grpSize)%MOD)%MOD;
	
	return  dp[idx][grpSize] = totalfitness;
}
