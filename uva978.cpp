...
int main()
{
   ...
   freopen("input.txt", "r", stdin);
	...
	int N;
	cin>>N;
	multiset<int,greater<int> >gren;
	multiset<int,greater<int> >blue;
	vector<int>grenA,blueA;
	vector<multiset<int,greater<int> >::iterator>grenD,blueD;
	int B,SG,SB,tmp;

	while(N--){
		cin>>B>>SG>>SB;
		gren.clear();blue.clear();
		FOR(i,0,SG){
			cin>>tmp;
			gren.insert(tmp);
		}
...
//		cout<<gren.size()<<" "<<blue.size()<<endl;
		while(!gren.empty() && !blue.empty()){
			grenD.clear();blueD.clear();grenA.clear();blueA.clear();
			

			it1 = gren.begin();it2 = blue.begin();
			for(int i=0; i<B && it1!=gren.end() && it2!=blue.end();i++,it1++,it2++){
				int t1 = *it1,t2 =  *it2;
				grenD.pb(it1);blueD.pb(it2);
				if(t1>t2){
					grenA.pb(t1-t2);
				}
				if(t2>t1){
			...
			for(int i=0;i<grenD.size();i++)
				gren.erase(grenD[i]);
			for(int i=0;i<blueD.size();i++)
				blue.erase(blueD[i]);
			for(int i=0;i<grenA.size();i++)
				gren.insert(grenA[i]);
			for(int i=0;i<blueA.size();i++)
				blue.insert(blueA[i]);

		}

		if(gren.empty()&&blue.empty())
