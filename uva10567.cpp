...
the following lines in main function.*/
	...
	while(cin>>S){
		...
		while(Q--){
			...
			FOR(i,0,SS.size()){
				int first_char = (int)SS[i];
				int bucket;

				if(first_char>=97)
					bucket = first_char-'a'+SMALL;
				else
					bucket = first_char-'A'+CAPITAL;

					auto tmp = upper_bound(vv[bucket].begin(),vv[bucket].end(),lastPtr);
					if(tmp==vv[bucket].end()){
						valid = false;
					}
					else{
						lastPtr = *tmp;
			...
			if(valid)
				cout<<"Matched "<<firstPtr<<" "<<lastPtr<<endl;
			else
