...
ll getTotalUnits(ll totalCost)
{
	ll cnt = 0;

	ll tmp = min(2*100LL,totalCost);
	cnt+=tmp/2;
	totalCost-=tmp;

	tmp = min(3*9900LL,totalCost);
	cnt+=tmp/3;
	totalCost-=tmp;

	tmp = min(5*990000LL,totalCost);
	cnt+=tmp/5;
	totalCost-=tmp;
...
ll calculateCost(ll unit)
{
	ll cost = 0;

	ll tmp = min(100LL,unit);
	unit-=tmp;
	cost+=2*tmp;
...
ll validate(ll my,ll neighbour)
{
	ll myCost = calculateCost(my);
	ll neghCost = calculateCost(neighbour);



	// cout<<myCost<<" "<<neghCost<<endl;
	ll diff = neghCost-myCost;
	// cout<<diff<<" "<<B<<endl;
	return B-diff;
}


ll binarySearchAnswer(ll low, ll high,ll total)
{
	...
	while(low<high){
		...
		if(ev==0){
			...
			// cout<<ev<<" "<<"Lower bound increase"<<endl;
			low = mid+1;
		}
		else {
			// cout<<ev<<" "<<"Upper bound decrease"<<endl;
			high=mid-1;
		}
	}

	if(validate(mid,total-mid)==0)
		ans = mid;
	else if(validate(low,total-low)==0)
		ans = low;
...
the following lines in main function.*/
	...
	while(cin>>A>>B){
		if(!A&&!B)break;
		cout<<calculateCost(binarySearchAnswer(0,getTotalUnits(A),getTotalUnits(A)))<<endl;

		// cout<<getTotalUnits(A)<<endl;
	}
