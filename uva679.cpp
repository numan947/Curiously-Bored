...
int findEndNode(int curNode, int totalBalls)
{
	//cout<<curNode<<endl;
	int left = curNode*2, right = left+1;
	//cout<<maxNode<<endl;
	if(left<maxNode && right<maxNode){
		if(totalBalls%2)
			return findEndNode(left,totalBalls/2 +1);
		else
			return findEndNode(right,totalBalls/2);
	}
