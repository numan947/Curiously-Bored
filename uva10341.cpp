...
long double findValue(double left, double right)
{
	long double mid,ans=-INF;
	while(fabs(right-left)>EPS){
		mid = (left+right)/2.0;
		// cout<<evaluate(mid)<<endl;
		if(LTE(evaluate(mid),0+EPS)){
			right = mid;
			ans = mid;
		}else
			left = mid;
...
the following lines in main function.*/
	...
	while(cin>>p>>q>>r>>s>>t>>u){
		double ans = findValue(0,1);
		double a = Roundoff(evaluate(ans),10000);
		if(!EQ(a,0)){
			cout<<"No solution"<<endl;
		}
		else
			cout<<prec(4)<<ans<<endl;
	}
