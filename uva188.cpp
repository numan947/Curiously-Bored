...
ll strToInt(string s)
{
	...
	for(int i=s.size()-1;i>=0;i--){
		//cout<<s[i]-'a'+1<<"....Adding......"<<curMul*(s[i]-'a'+1)<<endl;
		total+=curMul*(s[i]-'a'+1);
		curMul<<=5;
	}
...
the following lines in main function.*/
	...
	while(getline(cin,inp)){
		...
		for(int i=0;i<toks.size();i++){
			//cout<<strToInt(toks[i])<<endl;
			ll tmp = strToInt(toks[i]);
			vv.pb(tmp);
		}
		...
		for(int c=1;!ans;){
			...
			for(int i=0;i<n;i++){
				for(int j=i+1;j<n;j++){
					if((c/vv[i])%n==(c/vv[j])%n){
						ll valI = ((c/vv[i])+1)*vv[i];
						ll valJ = ((c/vv[j])+1)*vv[j];
						conflicts.pb(min(valI,valJ));
						conflict=true;
					}
