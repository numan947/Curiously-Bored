...
int costGrid[MAX][MAX];
int maxCost;
bool row[MAX],diag1[MAX],diag2[MAX];


bool place(int r, int c)
{
	//cout<<"Checking<<"<<r<<" "<<c<<endl;
	return !(row[r] || diag1[r+c] ||diag2[r-c+8-1]);
}

void backtrack(int c, int cst)
{
	if(c==8){
		//cout<<getCost()<<endl;
...
the following lines in main function.*/
	...
	while(k--){
		...
		cout<<std::right<<setw(5)<<maxCost<<endl;
	}
