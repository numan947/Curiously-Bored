...
//number of primes less than 100 is around 170
ll dp[300][MAX][20];
ll n,k;
vector<int>prime;
...
int solveDP(int idx,int remK, int remN)
{
	// cout<<remK<<" "<<remN<<" "<<prime[idx]<<endl;
	if(remK==0 && remN==0)
		return 1;
	else if(remK<=0||remN<=0||idx>=prime.size()||idx<0)
		return 0;
	...
	int tmp1 = 0,tmp2 = 0;
	if(remN>=prime[idx])
		tmp1 = solveDP(idx-1,remK-1,remN-prime[idx]);
	tmp2 = solveDP(idx-1,remK,remN);

	dp[idx][remN][remK] = tmp1 + tmp2;
...
the following lines in main function.*/
	...
	while(cin>>n>>k){
		if(!n&&!k)
			break;
		ms(dp,-1);
		int idxMax = lower_bound(prime.begin(),prime.end(),n) - prime.begin();
		cout<<solveDP(idxMax,k,n)<<endl;
	}
