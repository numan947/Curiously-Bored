...
the following lines in main function.*/
	...
	while(N--){
		...
		for(int i=1;i<=dress[0][0];i++){
			if(M-dress[0][i]>=0)
				reachable[0][M-dress[0][i]] = 1;
		}
		for(int i=1;i<C;i++){
			for(int money=0;money<M;money++){
				if(reachable[i-1][money]){
					for(int j=1;j<=dress[i][0];j++){
						if(money-dress[i][j]>=0){
							// cout<<i<<" "<<money<<" "<<dress[i][j]<<endl;
							reachable[i][money-dress[i][j]] = 1;
							// cout<<reachable[i][money-dress[i][j]]<<endl;
						}
					}
				}
			}
		...
		int targetMoney;
		for(targetMoney=0;targetMoney<=M && !reachable[C-1][targetMoney];targetMoney++);
		// cout<<targetMoney<<endl;

		if(targetMoney==M+1)
			cout<<"no solution"<<endl;
		else
			cout<<M-targetMoney<<endl;
