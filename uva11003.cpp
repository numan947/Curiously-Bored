...
int findOptimal(int idx, int currentLim)
{
	if(idx==N||currentLim==0)
		return 0;
	if(currentLim<0)
		return	-INF;
	
	if(dp[idx][currentLim]!=-1)
		return dp[idx][currentLim];
	
	int t1 = 0,t2 = 0;

	if(box[idx].fi<=currentLim)
		t1 = 1+findOptimal(idx+1,min(currentLim-box[idx].fi,box[idx].se));
	
	t2 = findOptimal(idx+1,currentLim);

	return dp[idx][currentLim] = max(t1,t2);
}
...
the following lines in main function.*/
	...
	while(cin>>N){
		...
		int mx = 0;
		for(int i=0;i<N;i++){
			if(curMaxCap<box[i].se){
				curMaxCap = box[i].se;
				ms(dp,-1);
				mx = max(mx,1+findOptimal(i+1,box[i].se));
			}
		}
