...
void rotate(int n){
	string tmp[n];
	for(int i=0;i<n;i++)tmp[i]=sml[i];
	for(int i=0;i<n;i++)
		for(int j=0;j<n;j++)
			tmp[j][n-i-1]=sml[i][j];
	for(int i=0;i<n;i++)sml[i]=tmp[i];
}
int main()
{
	...
	while(true){
		scanf("%d %d ",&N,&n);
		if(!N&&!n)break;
		FOR(II,0,N)cin>>big[II];
		FOR(II,0,n)cin>>sml[II];
		cout<<countInBig(N,n);
		FOR(II,0,3){
			rotate(n);
			cout<<" "<<countInBig(N,n);
