...
the following lines in main function.*/
	...
	while(cin>>N>>Q){
		...
		posMap.clear();
		FOR(it,0,N){
			if(!mapHas(posMap,marbles[it]))
				posMap[marbles[it]] = it+1;
		}
		...
		FOR(i,0,Q){
			cin>>qq;
			// int pos = lower_bound(marbles,marbles+N,qq)-marbles;
			// if(marbles[pos]==qq){
			// 	cout<<qq<<" found at "<<pos+1<<endl;
			// }
			// else{
			...
			if(mapHas(posMap,qq)){
				cout<<qq<<" found at "<<posMap[qq]<<endl;
			}
			else{
				cout<<qq<<" not found"<<endl;
			}
