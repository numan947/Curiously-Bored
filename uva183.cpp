...
char checkMatrix(pii topLeft, pii bottomRight)
{
	int _1,_0;
	_1=_0=0;
...
void DecomposeBitMap(pii topLeft,pii bottomRight)
{
	if(topLeft.first==bottomRight.first||topLeft.second==bottomRight.second)
		return;
	char ch = checkMatrix(topLeft,bottomRight);
	decomposed+=ch;
	
	// cout<<"<"<<topLeft.first<<","<<topLeft.second<<"> <"<<bottomRight.first<<","<<bottomRight.second<<">"<<endl;
	// cout<<ch<<endl;
	...
	int midr = ceil((1.0*(topLeft.first+bottomRight.first))/2.0);
	int midc = ceil((1.0*(topLeft.second+bottomRight.second))/2.0);
	pii mid = mp(midr,midc);
	
	DecomposeBitMap(topLeft,mid);
	// if(abs(topLeft.first-mid.first)>0)
	DecomposeBitMap(mp(topLeft.first,mid.second),mp(mid.first,bottomRight.second));
	// if(abs(topLeft.second-mid.second)>0)
	DecomposeBitMap(mp(mid.first,topLeft.second),mp(bottomRight.first,mid.second));
	DecomposeBitMap(mid,bottomRight);
}

void fillMatrix(pii topLeft, pii bottomRight,bool val)
{
	for(int i=topLeft.first;i<bottomRight.first;i++)
		for(int j=topLeft.second;j<bottomRight.second;j++)
			grid[i][j]=val;
...
void ReconstructBitMap(pii topLeft,pii bottomRight)
{
	if(topLeft.first==bottomRight.first||topLeft.second==bottomRight.second)
		return;
	++idx;
	if(decomposed[idx]!='D'){
		fillMatrix(topLeft,bottomRight,decomposed[idx]-'0');
		return;
	}
	...
	ReconstructBitMap(topLeft,mid);
	// if(abs(topLeft.first-mid.first)>0)
	ReconstructBitMap(mp(topLeft.first,mid.second),mp(mid.first,bottomRight.second));
	// if(abs(topLeft.second-mid.second)>0)
	ReconstructBitMap(mp(mid.first,topLeft.second),mp(bottomRight.first,mid.second));
	ReconstructBitMap(mid,bottomRight);
...
the following lines in main function.*/
	...
	while(true){
		...
		string tmpInp,ttmpInp;
		while(getline(cin,ttmpInp)){
			tokenize(ttmpInp,toks," ");
			if(toks.size()>1||toks[0]=="#")
				break;
			tmpInp+=ttmpInp;
		}
		inp = ttmpInp;
		...
		if(!format){
			...
			for(int i=0;i<r;i++){
				for(int j=0;j<c;j++){
					grid[i][j]=tmpInp[it++]-'0';
					// cout<<grid[i][j];
				}
				// cout<<endl;
			}
			decomposed="";
			DecomposeBitMap(mp(0,0),mp(r,c));
			...
			for(int i=0;i<decomposed.size();i++){
					cout<<decomposed[i];
					cnt++;
					if(cnt%50==0 && i!=decomposed.size()-1){
			...
			ReconstructBitMap(mp(0,0),mp(r,c));
