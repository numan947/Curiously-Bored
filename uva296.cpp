...
{	
	...
	for(int i=0;i<gList.size();i++){
		...
		int cg1 = 0;
		for(int j=0;j<4;j++)
			cg1+=(current[j]==cur.guess[j]);

		if(cg1!=(cur.report[0]-'0')){
			//y=false;
			return false;
		}
		...
		for(int j=0;j<4;j++){
			bool notFound=true;
			for(int k=0;notFound&&k<4;k++){
				if(taken[k])continue;
				else if(cur.guess[k]==current[j]){
					notFound=false;
					taken[k]=1;
				}
			}
	...
	//	cout<<y<<endl;
		cg2-=cg1;
		if(cg2!=(cur.report[2]-'0')){
			//y=false;
			return false;
		}
	//	cout<<current<<" "<<cg1<<"/"<<cg2<<"-->"<<cur.guess<<" "<<cur.report<<endl;
		//cout<<y<<endl;
	}
