...
void findMaxPossible(int currentNumber, int p)
{
	if(p==N)
		return;
	
	mx = max(mx,currentNumber);

	if(peg[p].empty()){
		peg[p].pb(currentNumber);
		return findMaxPossible(currentNumber+1,p);
	}

	for(int i=0;i<=p;i++){
		if(pegIsOk(i,currentNumber)){
			peg[i].pb(currentNumber);
			findMaxPossible(currentNumber+1,p);
			peg[i].pop_back();
			return;
		}
	}
	findMaxPossible(currentNumber,p+1);
}
