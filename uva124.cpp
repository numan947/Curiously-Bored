...
map<int,char>rvrsMap;
int ID;
int N;
vector<int>AdjList[MAX];
map<int,int>InEdgeCount;


int getID(char ch)
{
	if(!mapHas(globalMap,ch)){
		globalMap[ch] = ID;
		rvrsMap[ID] = ch;
		ID++;
	}
...
void generatePerms(string curS)
{
	if(curS.size()==N){
		cout<<curS<<endl;
		return;
	...
	for(int i=0;i<N;i++){
		if(!taken[i] && !InEdgeCount[i]){

			for(int j=0;j<AdjList[i].size();j++)
				InEdgeCount[AdjList[i][j]]--;
			taken[i] = 1;
			generatePerms(curS+rvrsMap[i]);
			taken[i] = 0;
			for(int j=0;j<AdjList[i].size();j++)
				InEdgeCount[AdjList[i][j]]++;
		}
...
the following lines in main function.*/
	...
	while(getline(cin,inp)){
		...
		for(int i=0;i<toks.size();i+=2){
			int u = globalMap[toks[i][0]];
			int v = globalMap[toks[i+1][0]];
			AdjList[u].pb(v);
			InEdgeCount[v]++;
		}
		generatePerms("");
