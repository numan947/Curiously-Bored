...
int main()
{
	...
	while(cin>>N>>M){
		if(!N&&!M)break;
		Jack.clear();Jill.clear();ans.clear();
		for(ll i=0;i<N;i++){
			cin>>tmp;
			Jack.insert(tmp);
		}
		...
		set_intersection(Jack.begin(),Jack.end(),Jill.begin(),Jill.end(),inserter(ans,ans.begin()));
...
//    freopen("output.txt", "w", stdout);
	...
	while(true){
		...
		while(M--){
			scanf("%lld",&x);
			if(binary_search(vct.begin(),vct.end(),x))count++;
		}
		cout<<count<<endl;
