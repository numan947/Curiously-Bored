...
bool FUCKTHISIAMOUT(contestant a, contestant b){
...
int main()
{
	...
	while(tc--){
		...
		while(getline(cin,input)){
			...
			//cout<<"HELLO WORLD"<<tmp.size()<<endl;
			int teamNum = stoi(tmp[0]);
			int probNum = stoi(tmp[1]);
			int subTime = stoi(tmp[2]);

			if(mp.find(teamNum)==mp.end()){
				contestant cc;
				cc.teamNum = teamNum;
				cc.penaltyTime = 0;
				cc.numSolved = 0;
				FOR(i,0,10)cc.problem[i]=0;
				mp[teamNum] = cc;

			}

			if(tmp[3]=="I" && mp[teamNum].problem[probNum-1]!=-1){
				mp[teamNum].problem[probNum-1]++;

			}
			else if(tmp[3]=="C" && mp[teamNum].problem[probNum-1]!=-1){
				mp[teamNum].numSolved++;
				mp[teamNum].penaltyTime+=(subTime)+20*(mp[teamNum].problem[probNum-1]);
				mp[teamNum].problem[probNum-1]=-1;
			}
		}
		...
		sort(vv.begin(),vv.end(),FUCKTHISIAMOUT);
		for(int i=0;i<vv.size();i++)
			printf("%d %d %d\n",vv[i].teamNum,vv[i].numSolved,vv[i].penaltyTime);
		if(tc)printf("\n");
