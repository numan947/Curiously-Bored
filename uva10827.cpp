...
/*		for(int i=0;i<2*N;i++){
			for(int j=0;j<2*N;j++){
				for(int k=i;k<min(i+N,2*N);k++){
					for(int l=j;l<min(j+N,2*N);l++){
						ll tmp = grid[k][l];
						if(i)
							tmp-=grid[i-1][l];
						if(j)
						...
						if(tmp>mx){
							...
							cout<<i<<","<<j<<endl;
							cout<<k<<","<<l<<endl;
							cout<<"actual   : "<<grid[k][l]<<endl;
							cout<<"sub-i-1  : "<<grid[i-1][l]<<endl;
							cout<<"sub-j-1  : "<<grid[k][j-1]<<endl;
							cout<<"add-i,j-1: "<<grid[i-1][j-1]<<endl;
							cout<<tmp<<endl;
						}
					}
				}
