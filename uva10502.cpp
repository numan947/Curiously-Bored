...
the following lines in main function.*/
	...
	while(cin>>r){
		...
		for(int szR=1;szR<=r;szR++){
			for(int szC=1;szC<=c;szC++){
				for(int sr=0;sr<r-szR+1;sr++){
					for(int sc=0;sc<c-szC+1;sc++){
						bool gr=1;
						for(int i=sr;gr&&i<r&&i<(sr+szR);i++){
							for(int j=sc;gr&&j<c&&j<(sc+szC);j++){
								gr = grid[i][j];
							}
						}
						if(gr)
