...
vector<int>edges[30];
map<char,int>encoder;
int ID;
int getId(char ch)
{
	if(encoder.find(ch)==encoder.end())
		encoder[ch]=ID++;
	return encoder[ch];
}
...
//    freopen("output.txt", "w", stdout);
	...
	while(scanf("%d",&N)==1){
		...
		//make union set
		u.unionSet(getId(ss[0]),getId(ss[1]));
		u.unionSet(getId(ss[1]),getId(ss[2]));
		// u.unionSet(getId(b),getId(c));
		sampleWake = u.findSet(getId(ss[0]));

		for(int i=0;i<M;i++){
			getline(cin,ss);
			int id1 = getId(ss[0]);
			int id2 = getId(ss[1]);
			//cout<<ss[0]<<" "<<id1<<" "<<ss[1]<<" "<<id2<<endl;
			edges[id1].pb(id2);
			edges[id2].pb(id1);
		}
		...
		while(u.numDisjointSets()>1){
			...
			for(int i=0;i<N;i++){
				...
				if(u.isSameSet(i,sampleWake)) //already wake
					continue;
			...
			if(toWake.size()){
				for(int i=0;i<toWake.size();i++){
					//db(toWake[i]);
					u.unionSet(toWake[i],sampleWake);
				}
				toWake.clear();
				years++;
			}
