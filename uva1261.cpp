...
#define MAX 12
string input;
void parseInput(string in, vector<pii>&vv)
{
	int be,ed;
	be=ed=0;
...
bool is_mergable(pii a, pii b){
	return input[a.se-1]==input[b.fi];
}


bool solve(vector<pii>currentRanges)
{
	...
	for(int i=0;i<currentRanges.size();i++){
		if((currentRanges[i].se-currentRanges[i].fi)<2)
			continue;
		vector<pii>tmp;
		...
		tmp.clear();
		if(i==0){
			for(int j=1;j<currentRanges.size();j++)
				tmp.pb(currentRanges[j]);
			is_solvable |= solve(tmp);
		}
		if(is_solvable)
			break;
		...
		if(i==currentRanges.size()-1){
			for(int j=0;j<currentRanges.size()-1;j++)
				tmp.pb(currentRanges[j]);
			is_solvable |= solve(tmp);
		}
		if(is_solvable)
		...
		//case3: merge case
		tmp.clear();
		if(i!=currentRanges.size()-1&&i!=0){
			if(is_mergable(currentRanges[i-1],currentRanges[i+1])){
				int j=0;
				while(j<i-1){
					tmp.pb(currentRanges[j]);
					j++;
				...
				tmp.pb(mp(currentRanges[i-1].fi,currentRanges[i+1].se));


				j=i+2;
				while(j<currentRanges.size()){
					tmp.pb(currentRanges[j]);
					j++;
				}
			}else{
