...
string grid[MAX];
int coloredGrid[MAX][MAX];

map<int,set<int> >hyaroglyphNeighbourMap;
set<int>taken;
...
void paint(int r, int c, int color)
{
	...
	for(int i=0;i<4;i++){
		newR = r + dr[i];
		newC = c + dc[i];
		if(newR>=0&&newR<H&&newC>=0&&newC<W && grid[newR][newC]==grid[r][c] && coloredGrid[newR][newC]==-1 )
			paint(newR,newC,color);
	}
}


void collectNeighbours(int r, int c)
{
	int newR,newC;
	...
	for(int i=0;i<4;i++){
		newR = r + dr[i];
		newC = c + dc[i];
		if(newR>=0&&newR<H&&newC>=0&&newC<W && coloredGrid[newR][newC]!=0 && coloredGrid[newR][newC]!=coloredGrid[r][c] && !setHas(taken,coloredGrid[newR][newC])){
			hyaroglyphNeighbourMap[coloredGrid[r][c]].insert(coloredGrid[newR][newC]);
			taken.insert(coloredGrid[newR][newC]);
		}
	}
}
...
the following lines in main function.*/
	...
	while(cin>>H>>W){
		...
		ms(coloredGrid,-1);
		taken.clear();
		hyaroglyphNeighbourMap.clear();

		FOR(i,0,H){
			FOR(j,0,W){
				if(coloredGrid[i][j]==-1){
					paint(i,j,cc_num);
					if(grid[i][j]=='1'){
						hyaroglyphNeighbourMap[cc_num] = set<int>();
					}
					cc_num++;
				}
			}
		...
		FOR(i,0,H){
			FOR(j,0,W){
				if(grid[i][j]=='1'){
					collectNeighbours(i,j);
				}
			}
		}

		string ans = "";
		FORall(it,hyaroglyphNeighbourMap){
			// cout<<it->fi<<" "<<(it->se).size()<<endl;
			switch ((it->se).size())
			{
				case 0:
					ans+="W";
