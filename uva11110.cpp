...
int getCCSize(int r, int c, int val)
{
	vis[r][c] = 1;
	int newR,newC;
	int cnt = 0;
	...
	for(int i=0; i<4; i++){
		...
		if(newR>=0 &&newR<n && newC>=0 && newC<n && !vis[newR][newC] && grid[newR][newC]==val)
			cnt+= getCCSize(newR,newC,val);
	}
...
the following lines in main function.*/
	...
	while(true){
		...
		FOR(i,0,n){
			FOR(j,0,n){
				if(!vis[i][j]){
					int ccSize = getCCSize(i,j,grid[i][j]);
					cc.push_back(ccSize);
				}
			}
		}
