...
void DijkstraLazyDeletion(int src)
{
	...
	while(!pq.empty()){
		...
		if(dist[u]<d) //LAZY DELETION REDUNDANCY
			continue;
...
the following lines in main function.*/
	...
	while(cin>>n>>m){
		...
		while(m--){
			cin>>a>>b>>l;
			AdjList[a-1].pb(mp(b-1,l));
			AdjList[b-1].pb(mp(a-1,l));
		}

		// for(int i=0;i<n;i++){
		// 	cout<<i<<" -->";
		// 	for(int j=0;j<AdjList[i].size();j++){
		// 		cout<<" <"<<AdjList[i][j].fi<<","<<AdjList[i][j].se<<">";
		// 	}
		// 	cout<<endl;
		// }

		DijkstraLazyDeletion(0);

		double ans=-INF;int max1_idx=-1;
		...
		for(int i=0;i<n;i++){
			int u=i;
			for(int j=0;j<AdjList[i].size();j++){
				int v = AdjList[i][j].fi;
				int w = AdjList[i][j].se;
		...
		cout<<"System #"<<++KS<<endl;
		if(max2_idx==-1){
			cout<<"The last domino falls after "<<prec(1)<<ans<<" seconds, at key domino "<<max1_idx+1<<"."<<endl;
		}else{
		
			// cout<<falltime<<endl;
			cout<<"The last domino falls after "<<prec(1)<< ans <<" seconds, between key dominoes "<<1+max2_idx<<" and "<<max1_idx+1<<"."<<endl;
		}
