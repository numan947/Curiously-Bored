...
{
	if(s1.size()!=s2.size()){
		cout<<"SIZE MISMATCH HAMMING DISTANCE"<<endl;
		return -1;
	}
	int len = s1.size();
	int cnt=0;
...
the following lines in main function.*/
	...
	while(cin>>M>>Q){
		...
		while(Q--){
			...
			for(int i=0;i<dbS;i++){
				int lenD = database[i].size();
				//cout<<"Processing string "<<i<<endl;
				if(lenD<lenT)
					continue;
				
				for(int j=0;j<lenD-lenT+1;j++){
					int tmpCount=0;
					for(int k=0;k<lenT;k++)
						tmpCount+=(t[k]!=database[i][j+k]);
					if(tmpCount<minMismatch){
						minMismatch = tmpCount;
						ans = i+1;
					}
					//cout<<j<<"  "<<tmpCount<<endl;
				}
			}
			cout<<ans<<endl;
		}
