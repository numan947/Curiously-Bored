...
SegmentTreeNode merge(SegmentTreeNode a, SegmentTreeNode b){
	int matched = min(a.openU,b.closedU);

	return {a.openU+b.openU-matched,a.closedU+b.closedU-matched};
}
...
void update_tree(int nd, int l, int r, int i, int j, int val)
{
	...
	if(l==r){ //reached node
		...
		//tree[nd] += val
		tree[nd].openU = 1 - tree[nd].openU;
		tree[nd].closedU = 1 - tree[nd].closedU;
...
SegmentTreeNode query_tree(int nd, int l, int r, int i, int j)
{
	...
	SegmentTreeNode ans = merge(q1,q2);
	return ans;
}
...
the following lines in main function.*/
	...
	while(scanf("%d",&n)==1){
		...
		while(m--){
			scanf("%d",&p);
			if(p){
				update_tree(1,0,n-1,p-1,p-1,1);
			}
			else{
				SegmentTreeNode tmp = query_tree(1,0,n-1,0,n-1);

				if(!tmp.openU && !tmp.closedU)
					printf("YES\n");
				else
					printf("NO\n");
			}
