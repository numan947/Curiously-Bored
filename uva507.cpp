...
the following lines in main function.*/
	...
	while(TC--){
		...
		FOR(i,0,s-1){
			...
			// cout<<tmp<<" ";
			currentNiceness+=tmp;
			// cout<<currentNiceness<<endl;
			if(currentNiceness>=0)
				curR++;
			else{
				currentNiceness = 0;
				curL=curR=i+1;
			}
			if(currentNiceness>mxNiceness||(currentNiceness==mxNiceness && (curR-curL+1)>(mxR-mxL+1))){
				mxNiceness = currentNiceness;
				mxL = curL+1;
				mxR = curR+1;
			}
		}
		...
		if(mxNiceness>0)
			cout<<"The nicest part of route "<<++R<<" is between stops "<<mxL<<" and "<<mxR<<endl;
		else
