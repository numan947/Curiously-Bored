...
int coins[14] = {10000,5000,2000,1000,500,200,100,50,20,10,5};

ull dp[14][30005];
bool vis[14][30005];


ull countWays(ll idx, ll rem)
{
	...
	if(coins[idx]<=rem){
		for(int i=1;;i++){
			if(coins[idx]*i>rem)
				break;
			cnt+=countWays(idx+1,rem-coins[idx]*i);
		}
	}
	cnt+=countWays(idx+1,rem);
	
	vis[idx][rem] = 1;
	return dp[idx][rem] =  cnt;
}
...
the following lines in main function.*/
	...
	while(cin>>in){
		...
		cout<<setw(6)<<prec(2)<<Roundoff(in,100)<<setw(17)<<countWays(0,1.0*in*100.0)<<endl;
