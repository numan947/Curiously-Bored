...
int countArea(int sr, int sc)
{
	...
	for(int i=0;i<4;i++){
		newR = sr+dr[i];
		newC = sc+dc[i];
		// cout<<newR<<" "<<newC<<" "<<grid[newR][newC]<<endl;
		if(newR>=0&&newR<R&&newC>=0&&newC<C&&(!vis[newR][newC])&&grid[newR][newC]=='0')
			cnt+=countArea(newR,newC);
	}
	return cnt;
}
