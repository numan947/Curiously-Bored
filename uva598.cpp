...
void solve(int setSize, int idx, int mask)
{
	if(setSize<0||idx>newspapers.size())
		return;
	if(setSize == 0){
		int i = 0;
		while(i<newspapers.size()){
			if(bit(mask,i)){
				cout<<newspapers[i++];
				break;
			}
	...
	solve(setSize-1,idx+1, mask|(1<<idx));
	solve(setSize, idx+1,mask);
}
