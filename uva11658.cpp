...
double findOptimalAnswer(int idx, int taken)
{
	if(taken>5000){
		return 100.0*xshare/taken;
	}
	if(idx==share.size())
		return -INF;
	...
	if(st[idx][taken])
		return dp[idx][taken];
	st[idx][taken] = 1;
	return dp[idx][taken] = max(findOptimalAnswer(idx+1,taken),findOptimalAnswer(idx+1,taken+share[idx]));
}
...
the following lines in main function.*/
	...
	while(cin>>n>>x){
		...
		FOR(i,0,n){
			...
			dtmp+=EPS;
			if(i==x-1){
				xshare = (int)((double)dtmp*100.0);
				// cout<<xshare<<" ";
			}
			else{
				// cin>>dtmp;
				share.pb((int)(dtmp*100.0));
			}
// share.pb((int)(dtmp*100.0));
			// cout<<dtmp*100.0<<" ";
			// vtmp+=dtmp*100.0;
...
/* 		int tmp = 0;
		...
		cout<<endl;
		cout<<tmp+xshare<<endl;
		cout<<vtmp<<endl; */
		ms(st,0);
		// cout<<findOptimalAnswer(0,xshare)<<endl;
		cout<<prec(2)<<Roundoff(findOptimalAnswer(0,xshare),100)<<endl;
