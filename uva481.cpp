...
void findLIS(int n)
{
	...
	for(int i=1;i<n;i++){
		for(int j=i-1;j>=0;j--){
			if(seq[j]<seq[i] && L[j].size()+1>L[i].size()){
				L[i] = L[j];
				L[i].pb(seq[i]);
			}
		}
...
the following lines in main function.*/
	...
	int L[MAX],L_id[MAX], P[MAX],lis=0,lis_end=0;

	for(int i=0;i<n;i++){
		int pos_i = lower_bound(L,L+lis,seq[i])-L;
		L[pos_i] = seq[i];
		L_id[pos_i]=i;
		P[i] = pos_i>0?L_id[pos_i-1]:-1;
		if(pos_i+1>=lis){
			lis = pos_i+1;
			lis_end = i;
		}
