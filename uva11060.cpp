...
int getID(string bevName1)
{
	if(!mapHas(forMap,bevName1)){
		forMap[bevName1] = ID;
		revMap[ID] = bevName1;
		ID++;
	}
	return forMap[bevName1];
}
...
the following lines in main function.*/
	...
	while(cin>>N){
		...
		FOR(i,0,M){
			cin>>bevName1>>bevName2;
			int u = getID(bevName1);
			int v = getID(bevName2);

			AdjList[u].pb(v);
			InDegree[v]++;
		}

		vector<int>drinkOrder = getOrder();
		

		cout<<"Case #"<<++KS<<": Dilbert should drink beverages in this order:";
		FOR(i,0,drinkOrder.size())
			cout<<" "<<revMap[drinkOrder[i]];
		cout<<"."<<endl<<endl;
