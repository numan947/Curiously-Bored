...
string apply_1(int c,string tmp)
{
	for(int i=0;i<c;i++){
		char t = tmp[4];
		tmp[4]=tmp[5];
...
string apply_2(int c,string tmp)
{
	for(int i=0;i<c;i++){
		char t = tmp[3];
		tmp[3]=tmp[5];
...
string apply_3(int c,string tmp)
{
	for(int i=0;i<c;i++){
		char t = tmp[2];
		tmp[2]=tmp[4];
...
the following lines in main function.*/
	...
	while(getline(cin,s)){
		...
		string c2 = s.substr(6);

		bool done=false;

		for(int i=0;!done&&i<4;i++){
			string tmp1 = apply_1(i,c1);
			for(int j=0;!done&&j<4;j++){
				string tmp2 = apply_2(j,tmp1);
				for(int k=0;!done&&k<4;k++){
					string tmp3 = apply_3(k,tmp2);
					if(tmp3==c2)
						done=true;
					//cout<<tmp<<endl;
				}
