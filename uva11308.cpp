...
bool customCmpr(psi &left, psi &right)
{
	if(left.se==right.se)return left.fi<right.fi;
...
//    freopen("output.txt", "w", stdout);
	...
	multimap<string,psi>recipeMap;
	vector<psi>possible;
	int t,m,n,b,qq,k;

	string name,tmp1,tmp2,tmp;
	getline(cin,tmp);
	tokenize(tmp,toks," ");
	t = stoi(toks[0]);
	...
	while(t--){
		...
		FOR(i,0,n){
			...
			FOR(kk,0,k){
				...
				tmp2 = toks[0];
				qq = stoi(toks[1]);
				//recipeMap.insert(mp(tmp1,mp(tmp2,qq)));
				pTotal+= qq*priceMap[tmp2];

			}
			if(pTotal<=b)
				possible.pb(mp(tmp1,pTotal));
		}

		sort(possible.begin(), possible.end(),customCmpr);
