...
void factorize(ll n)
{
	...
	// 	printf("%d ",allFactors[i] );
	// printf("\n");
}



void solve(int buffIdx,int curNum,int curFactor)
{
	if(curNum==1){
		vector<ll>bb;
	...
	for(;curFactor<allFactors.size();curFactor++){
		if(curNum%allFactors[curFactor]==0 && (curNum/allFactors[curFactor]>=allFactors[curFactor] || curNum==allFactors[curFactor]))
		{
			tmp[buffIdx] = allFactors[curFactor];
			solve(buffIdx+1,curNum/allFactors[curFactor],curFactor);
		}
	}
}
