...
//    freopen("output.txt", "w", stdout);
	...
	while(tc--){
		...
		int correctLength = 2*totalLength/frags.size();

		for(int i=0;i<frags.size();i++){
			for(int j=i+1;j<frags.size();j++){
				//cout<<frags[i]<<" "<<frags[j]<<endl;
				if(frags[i].size()+frags[j].size()==correctLength){
					mpCnt[frags[i]+frags[j]]++;
					mpCnt[frags[j]+frags[i]]++;
				}
			}
		}
