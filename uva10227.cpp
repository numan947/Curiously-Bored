...
bool consistent(vector<int>v1,vector<int>v2){
...
int main()
{
	...
	while(tc--){
		...
		while(getline(cin,ss)){
			...
			if(ss=="")break;
			tokenize(ss,toks," ");
			int people = stoi(toks[0]);
			int tree = stoi(toks[1]);
			if(find(vv[people-1].begin(),vv[people-1].end(),tree)==vv[people-1].end())
				vv[people-1].pb(tree);
		}
