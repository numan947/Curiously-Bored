...
int getSizeOfContinent(int r, int c)
{
	vis[r][c] = 1;
	...
	FOR(i,0,4){
		...
		if(newR>=0&&newR<R && newC>=0&&newC<C){
			if(grid[newR][newC]==land && !vis[newR][newC])
				cc+=getSizeOfContinent(newR,newC);
		}
	}
...
the following lines in main function.*/
	...
	while(cin>>R>>C){
		...
		// cout<<land<<endl;
		int mx =0; getSizeOfContinent(rr,cc);
		
		FOR(i,0,R){
			FOR(j,0,C){
				if(!vis[i][j] && grid[i][j]==land){
					int v = getSizeOfContinent(i,j);
					// cout<<i<<" "<<j<<" "<<v<<endl;
					mx = max(mx,v);
				}
			}
