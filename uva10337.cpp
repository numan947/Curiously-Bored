...
int solve(int curP,int curH)
{
	...
	if(dp[curP][curH]!=-1)
		return dp[curP][curH];
	//climb
	// vv.pb(60-w[curH][curP]);
	int choice1 = 60 - w[curH][curP] + solve(curP+1,curH+1);
	//hold
	// vv.pop_back();
	// vv.pb(30-w[curH][curP]);
	int choice2 = 30- w[curH][curP] + solve(curP+1,curH);
	//sink
	// vv.pop_back();
	// vv.pb(20-w[curH][curP]);
	int choice3 = 20- w[curH][curP] + solve(curP+1,curH-1);
	// vv.pop_back();
	return dp[curP][curH] = min(choice1,min(choice2,choice3));
}
