...
{
	return {a.sum+b.sum,a.ssum+b.ssum};
}
...
{
	if(checker==0){ //check if lazy update is needed for this node, pass 1 when lazy update is getting set
		tree[nd].sum = (r-l+1)*val; //update current node
		tree[nd].ssum = (r-l+1)*val*val;
		...
		if(r!=l){ //mark current node's children for lazy update
			...
			lazy[left(nd)].val = val;
			lazy[right(nd)].val = val;
		}
	}

	else if(checker==1){
		
		tree[nd].ssum+=tree[nd].sum*2*val+(r-l+1)*val*val;
		tree[nd].sum+=(r-l+1)*val;

		if(r!=l){ //mark current node's children for lazy update
			if(lazy[left(nd)].typ==-1)
				lazy[left(nd)].typ = 1;
			if(lazy[right(nd)].typ==-1)
				lazy[right(nd)].typ = 1;
...
void update_tree(int nd, int l, int r, int i, int j,int tp, ll val)
{

	//clear this node's lazy updates
	update_lazy(nd,l,r,lazy[nd].typ,lazy[nd].val);
	lazy[nd].typ = -1;
	lazy[nd].val = -1;
	...
	if(l>=i&&r<=j){ //reached node
		//UPDATE LAZILY...........
		update_lazy(nd,l,r,tp,val);
		/*********WRITE NOTHING BELOW THIS*************/
		return;
	}

	update_tree(left(nd),l,mid(l,r),i,j,tp,val);
	update_tree(right(nd),1+mid(l,r),r,i,j,tp,val);


	updateParent(nd,left(nd),right(nd));
}

STNode query_tree(int nd, int l, int r, int i, int j)
{
	//clear this node's lazy updates
	update_lazy(nd,l,r,lazy[nd].typ,lazy[nd].val);
	lazy[nd].typ = -1;
	...
	STNode q1 = query_tree(left(nd),l,mid(l,r),i,j);
	STNode q2 = query_tree(right(nd),1+mid(l,r),r,i,j);
...
/*	ios_base::sync_with_stdio(0);
	...
	while(T--){
		...
		printf("Case %d:\n",++KASE);
		while(Q--){
			scanf("%d %d %d",&cmd,&st,&ed);

			if(cmd==2){
				//query tree
				STNode tmp = query_tree(1, 0, N-1,st-1,ed-1);
				printf("%lld\n", tmp.ssum);
				...
				//update tree...add x
				scanf("%d",&x);
				update_tree(1,0,N-1,st-1,ed-1,cmd,x);
			}
			else{
				//update tree...set x
				scanf("%d",&x);
