...
int dp[MAX][10005];
// int dpRight[MAX][10000];
int car[MAX];
int p[MAX][10005];
int tc,cap,n,tmp;


int backtrack(int idx, int left, int right)
{
	if(left>cap||right>cap)
		return -INF;
	if(idx==n)
		return 0;
	if(left+car[idx]>cap && right+car[idx]>cap)
		return 0;
	
	int t1 = -INF,t2 = -INF;
	if(dp[idx][max(left,right)]!=-1)
		return dp[idx][max(left,right)];
	
	t1 = max(t1,backtrack(idx+1,left+car[idx],right));
	t2 = max(t2,backtrack(idx+1,left,right+car[idx]));

	dp[idx][max(left,right)] = 1+max(t1,t2);

	if(t1>=t2)
		p[idx][max(left,right)] = 1;
	else
		p[idx][max(left,right)] = -1;
...
the following lines in main function.*/
	...
	while(tc--){
		...
		int nn = backtrack(0,0,0);
		cout<<nn<<endl;

		int idx = 0,left=0,right=0;
		while(idx<nn && p[idx]!=0){
			if(p[idx][max(left,right)]==-1){
				cout<<"starboard"<<endl;
				right+=car[idx];
			}
			else if(p[idx][max(left,right)]==1){
				cout<<"port"<<endl;
				left+=car[idx];
			}
			idx++;
...
int n;
vector<char>tg;
int ferry(int i,int left,int right)
{
    if(i==n)return 0;
    ...
    if(left-cars[i]>=0){
        l1=1+ferry(i+1,left-cars[i],right);
    }
    //l2=ferry(i+1,left,right);
    //l1=max(l1,l2);

    if(right-cars[i]>=0){
        r1=1+ferry(i+1,left,right-cars[i]);
    }
  //  r2=ferry(i+1,left,right);
   // r1=max(r1,r2);
...
void recon(int i, int l, int r)
{
    if(bp[i][l]==-1)return;
    if(bp[i][l]==1){
        if(l-cars[i]>=0){
        printf("port\n");
        recon(i+1,l-cars[i],r);
        }
    }
    else{
        if(r-cars[i]>=0){
        printf("starboard\n");
        recon(i+1,l,r-cars[i]);
        }
    }
}
int main()
