...
void resultGenerator(int idx, string st)
{
	if(idx>inp.size())
		return;
	if(idx==inp.size()){
	...
	string d1 = inp.substr(idx,1);
	if(mapHas(revMap,d1)){
		resultGenerator(idx+1,st+revMap[d1]);
	}
	if(idx<inp.size()-1){
			string d2 = inp.substr(idx,2);
			if(mapHas(revMap,d2)){
				resultGenerator(idx+2,st+revMap[d2]);
			}
	}
