...
the following lines in main function.*/
	...
	while(cin>>R>>C>>S){
		...
		while(true){
			...
			if(traceGrid[curR][curC]!=-1){
				//repetition of the same place....loop
				stpCnt=traceGrid[curR][curC];
				loopLen = step-traceGrid[curR][curC];
				break;
			}
			traceGrid[curR][curC] = step++;
		...
		if(loopLen!=-1){
			cout<<stpCnt<<" step(s)";
			cout<<" before a loop of "<<loopLen<<" step(s)";
			cout<<endl;
		}else{
			cout<<stpCnt<<" step(s) to exit";
			cout<<endl;
		}
	}
