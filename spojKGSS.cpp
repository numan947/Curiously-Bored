...
{	
	// cout<<"Given"<<endl;
	// cout<<a.best<<" "<<a.sbest<<endl;
	// cout<<b.best<<" "<<b.sbest<<endl;
	SegmentTreeNode tmp;
	tmp.best = max(a.best,b.best);
	if(a.best==tmp.best)
		tmp.sbest = max(a.sbest,b.best);
	else
		tmp.sbest = max(b.sbest,a.best);
	// cout<<"calculated:"<<endl;
	// cout<<tmp.best<<" "<<tmp.sbest<<endl;
	return tmp;
}
	...
	while(scanf("%d",&N)==1){
		...
		while(Q--){
			...
			if(ch=='U')
				...
				SegmentTreeNode tmp = query_tree(1,0,N-1,a-1,b-1);
				printf("%lld\n",tmp.best+tmp.sbest);
			}
		}
	}
