...
#define MAX 2147484000


set<ll>allFactors;

ll csPow(int a, int b) //returns a^b
{
	ll ans = 1;
...
void generateFactors()
{
	for(ll i=0;i<31;i++){
		for(ll j =0;j<21;j++){
			ll tmp = csPow(2,i)*csPow(3,j);
			if(tmp<MAX)
				allFactors.insert(tmp);
		}
	}
...
// ll findUpperBound(ll lower,ll upper){
// 	ll mid,ans;
// 	while(lower<=upper){
...
the following lines in main function.*/
	...
	// cout<<findUpperBound(100,MAX)<<endl;
	generateFactors();
	...
	while(cin>>n){
		if(!n)break;
		auto it = lower_bound(allFactors.begin(),allFactors.end(),n);
		cout<<*it<<endl;
	}
