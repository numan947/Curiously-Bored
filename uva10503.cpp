...
bool findPath(int toMatch,int s)
{
	if(s==0){
		if(toMatch == e.fi)
			return true;
	...
	for(int i=0;i<m;i++){
		if(taken[i])
			continue;
		if(piece[i].fi == toMatch){
			taken[i] = 1;
			if(findPath(piece[i].se,s-1))
				return true;
			taken[i] = 0;
		}
		else if(piece[i].se == toMatch){
			taken[i] = 1;
			if(findPath(piece[i].fi, s-1))
				return true;
			taken[i] = 0;
		}
	}
