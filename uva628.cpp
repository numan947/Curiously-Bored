...
void generate(int ruleIt, string ans)
{
	if(ruleIt==rule.size()){
		cout<<ans<<endl;
		return;
	...
	if(rule[ruleIt] == '#'){
		for(int i=0;i<dict.size();i++)
			generate(ruleIt+1,ans+dict[i]);
	}
	else if(rule[ruleIt] == '0'){
		for(int i=0;i<10;i++){
			char ch = i+'0';
			generate(ruleIt+1,ans+ch);
		}
	}
}


// void generate(int ruleIt, int dictIt, int numsIt, string ans)
// {
// 	if(ruleIt==rule.size()){
// 		cout<<ans<<endl;
// 		return;
...
// 		if(dictIt >= dict.size())
// 			return;
// 		generate(ruleIt+1, dictIt, numsIt, ans+dict[dictIt]);
// 		generate(ruleIt, dictIt+1, numsIt, ans);
// 	}
// 	else if(rule[ruleIt] == '0'){
// 		if(numsIt>=10)
// 			return;
// 		char ch = numsIt+'0';
// 		generate(ruleIt+1, dictIt, numsIt, ans+ch);
// 		generate(ruleIt, dictIt, numsIt+1, ans);
// 	}
// }
