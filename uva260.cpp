...
bool go_black(int r, int c)
{
	...
	for(int i=0;i<6;i++){
		newR = r+dr[i];
		newC = c+dc[i];
		if(newR>=0 && newR<N && newC>=0 && newC<N && grid[newR][newC]=='b' && !vis[newR][newC]){
			if(go_black(newR,newC))
				return true;
		}
	}
	return false;
}
bool go_white(int r, int c)
{
	vis[r][c] = 1;
	if(c==N-1)
		return true;
	...
	for(int i=0;i<6;i++){
		newR = r+dr[i];
		newC = c+dc[i];
		if(newR>=0 && newR<N && newC>=0 && newC<N && grid[newR][newC]=='w' && !vis[newR][newC]){
			if(go_white(newR,newC))
				return true;
		}
	}
	return false;
