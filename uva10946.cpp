...
int getSize(int r, int c)
{
	...
	for(int i=0;i<4;i++){
		...
		if(newR>=0&&newR<R&&newC>=0&&newC<C&&grid[newR][newC]==grid[r][c] && !vis[newR][newC])
			sz += getSize(newR,newC);
	}

	return sz+1;
}

bool compr(pair<char,int>p1,pair<int,char>p2)
{
	if(p1.se==p2.se)
		return p1.fi<p2.fi;
	return p1.se>p2.se;
	...
	while(cin>>R>>C){
		...
		FOR(i,0,R){
			FOR(j,0,C){
				if(grid[i][j]!='.' && !vis[i][j]){
					int sz = getSize(i,j);
					hole.insert(mp(grid[i][j],sz));
				}
			}
		}
		cout<<"Problem "<<++KS<<":"<<endl;
		FORall(it,hole){
			vv.push_back(mp(it->fi,it->se));
		}

		sort(vv.begin(),vv.end(),compr);
