...
typedef struct pl{
	...
	int atk;
	int def;
	bool operator < (const struct pl &T) const
	{
		return name<T.name;
	}
...
the following lines in main function.*/
	...
	while(T--){
				...
				for(int k=j+1;k<10;k++){
					...
					for(int l=k+1;l<10;l++){
						...
						for(int m=l+1;m<10;m++){
							if(l==m)continue;
							int atkSum = v[i].atk+v[j].atk+v[k].atk+v[l].atk+v[m].atk;
							int defSum = 0;
							for(int p=0;p<10;p++)
								if(p!=i&&p!=j&&p!=k&&p!=l&&p!=m)
									defSum+=v[p].def;
							if(mxAtk==atkSum&&mxDef<defSum){
								mxDef = defSum;
								a1=i;a2=j;a3=k;a4=l;a5=m;
							}
							else if(mxAtk<atkSum){
								mxAtk = atkSum;
								mxDef = defSum;
								a1=i;a2=j;a3=k;a4=l;a5=m;
							}
		...
		cout<<"Case "<<++KS<<":"<<endl;
		cout<<"("<<v[a1].name<<", "<<v[a2].name<<", "<<v[a3].name<<", "<<v[a4].name<<", "<<v[a5].name<<")"<<endl;
		int d1,d2,d3,d4,d5;
		vs tmp;
		tmp.clear();
		for(int i=0;i<10;i++){
