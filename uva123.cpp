...
inline void tokenize(string str,vector<string> &tokens, string delim){ tokens.clear();size_t s = str.find_first_not_of(delim), e=s; while(s!=std::string::npos){e=str.find(delim,s);tokens.push_back(myLower(str.substr(s,e-s)));s=str.find_first_not_of(delim,e);}}
...
int main()
{
	...
	while(getline(cin,input)){
		...
		for(int i=0;i<tmp.size();i++){
			if(find(nonKey.begin(),nonKey.end(),tmp[i])==nonKey.end()){
				//cout<<tmp[i]<<endl;
				string ans="";
				if(i==0)ans+=myUpper(tmp[0]);
				else ans+=myLower(tmp[0]);
				for(int j=1;j<tmp.size();j++){
					ans+=" ";
					if(i==j)ans+=myUpper(tmp[j]);
					else ans+=myLower(tmp[j]);
				}
				AA aaa;
				aaa.ac=ans;
				aaa.cmp=myUpper(tmp[i]);
				aaa.pos=i;
				aaa.inputPos=pp;
				allOut.pb(aaa);
	...
	sort(allOut.begin(),allOut.end(),[](AA a,AA b){
		if(a.cmp==b.cmp){
			if(a.inputPos==b.inputPos)return a.pos<b.pos;
