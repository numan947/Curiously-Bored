...
void findWords(int tr, int tc, string curWord)
{
	curWord+=table[tr][tc];

	if(curWord.size()>=3){
		if(!vectHas(wordList,curWord))
			wordList.pb(curWord);
	}
	...
	for(int i = 0; i<8 ;i++){
		int nr = tr + dr[i];
		int nc = tc + dc[i];
		if(nr>=0&&nr<n&&nc>=0&&nc<n&&table[tr][tc]<table[nr][nc]){
			taken[nr][nc] = 1;
			findWords(nr,nc,curWord);
			taken[nr][nc] = 0;
		}
	}
}
...
the following lines in main function.*/
	...
	while(tc--){
		...
		sort(wordList.begin(), wordList.end(),cmpr);
