...
int getMaxFavour(int n, int m, bool e)
{
	if(e){
		if(m>mon)
			return -INF;
	...
	if ((m+item[n].fi)<=mon)
	{
		tmp = max(tmp, getMaxFavour(n - 1, m + item[n].fi, e)) + item[n].se;
	}

	tmp = max(tmp, getMaxFavour(n - 1, m, e));
...
the following lines in main function.*/
	...
	while (cin >> mon >> nn)
	{
		...
		FOR(i, 0, nn){
			...
			// cout<<item[i].fi<<" "<<item[i].se<<endl;
		}
		ms(dp, -1);
		int t1 = getMaxFavour(nn-1, 0,0);
		int t2 = -INF;
		if(mon>1800){
			mon+=200;
			ms(dp,-1);
			t2 = getMaxFavour(nn-1,0,1);
		}
