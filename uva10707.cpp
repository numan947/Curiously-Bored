...
multiset<multiset<int> >connt1,connt2;
multiset<int>tmp;

void getValue(bool grid[MAX][MAX],int r, int c)
{
	vist[r][c] = 1;
	...
	for(int i=0;i<4;i++){
		...
		if(newR>=H||newR<0||newC>=W||newC<0||grid[newR][newC]!=1)
			continue;
		setBit(msk,i);

		if(!vist[newR][newC])
			getValue(grid,newR,newC);
	}
