...
int orderDimSum(int idx,int moneyUsed,int foodCount)
{
	// cout<<moneyUsed<<endl;
	int moneySpentInGT = moneyUsed+(T*(N+1));
	moneySpentInGT+=ceil(1.0*moneySpentInGT*0.1L);

	if(moneySpentInGT>mxMoney||foodCount<0)
		return -INF;
	if(foodCount==0||idx==K||moneySpentInGT==mxMoney)
		return 0;
	
	if(dp[idx][moneyUsed][foodCount]!=-1)
		return dp[idx][moneyUsed][foodCount];
	
	return dp[idx][moneyUsed][foodCount] = max(dimsum[idx].se+orderDimSum(idx+1,moneyUsed+dimsum[idx].fi,foodCount-1),
	max(2*dimsum[idx].se+orderDimSum(idx+1,moneyUsed+2*dimsum[idx].fi,foodCount-2),orderDimSum(idx+1,moneyUsed,foodCount)));
}
...
the following lines in main function.*/
	...
	while(cin>>N>>x>>T>>K){
		...
		mxMoney = x*(N+1);
		int mxFood = 2*(N+1);
		cout<<prec(2)<<Roundoff((1.0*orderDimSum(0,0,mxFood))/(N+1),100)<<endl;	
	}
	return 0;
}
