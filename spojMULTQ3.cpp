...
SegmentTreeNode getLeaf()
{
	return {1,0,0};
}

SegmentTreeNode mergeNode(SegmentTreeNode a, SegmentTreeNode b)
{
...
void update_lazy(int nd, int l, int r,ll checker, ll val)
{
	if(checker){ //check if lazy update is needed for this node, pass 1 when lazy update is getting set
		if(val==1){
			int tmp = tree[nd].r0;
			tree[nd].r0 = tree[nd].r1;
			tree[nd].r1 = tree[nd].r2;
			tree[nd].r2 = tmp;
		}
		if(val==2){
			int tmp = tree[nd].r0;
			tree[nd].r0 = tree[nd].r2;
			tree[nd].r2 = tree[nd].r1;
			tree[nd].r1 = tmp;
		}
		if(l!=r){
			lazy[left(nd)]+=val;
...
void updateParent(int nd, int l, int r)
{
	...
	tree[nd] = mergeNode(tree[l],tree[r]);
}
...
/*	ios_base::sync_with_stdio(0);
	...
	while(Q--){
		scanf("%d %d %d",&cmd, &x, &y);
		if(cmd){
			SegmentTreeNode tmp = query_tree(1, 0, N-1, x,y);
			cout<<tmp.r0<<endl;
		}
		else
			update_tree(1,0,N-1,x,y,1);
	}
