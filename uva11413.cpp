...
bool isValid(int cap)
{
	...
	while(cont<m&&vess<n){
		...
		// cout<<cont<<" "<<vess<<endl;
		if(currentCont>=ara[vess]){
			currentCont-=ara[vess];
			// cout<<"Pouring in: "<<cont<<" Vess: "<<vess<<" Left Cap: "<<currentCont<<endl;
			vess++;
...
the following lines in main function.*/
	...
	while(cin>>n>>m){
		...
		FOR(i,0,n){
			cin>>ara[i];
			curTot+=ara[i];
			curMax=max(curMax,ara[i]);
		}
		
		// cout<<isValid(10)<<endl;	
		int correctCap = findCap(curMax,curTot*2);
		cout<<correctCap<<endl;
