...
//four direction moves: {up,left,down,right}-->anti-clockwise
...
int recurse(int index)
{
	...
	//print(index);
	string tmp[3];
	FOR(i,0,3)tmp[i]=ss[i];

	for(int i=0;i<3;i++){
		for(int j=0;j<3;j++){
			int cnt=0;
			//cout<<"determining "<<i<<","<<j<<","<<ss[i][j]<<endl;
			for(int k=0;k<4;k++){
				int ii=i+dx[k];
				int jj=j+dy[k];
			...
			//cout<<"determined "<<i<<","<<j<<","<<ss[i][j]<<endl;
		}
	}
	FOR(i,0,3)ss[i]=tmp[i];

	if(all_zero())return index;
