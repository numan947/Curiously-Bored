...
bitset<30>leftDiag,rightDiag,rw;
int row[MAX];

int solutions[100][MAX];
int sol;

bool placeable(int r, int c)
{
	return (!rw[r]) && (!leftDiag[r+c]) && (!rightDiag[r+7-c]);
}
...
void preCompute(int c)
{
	...
	for(int i=0;i<8;i++){
		if(placeable(i,c)){
			row[c]=i;
			rw[i]=leftDiag[i+c] = rightDiag[i+7-c] = 1;
			preCompute(c+1);
			rw[i]=leftDiag[i+c] = rightDiag[i+7-c] = 0;
		}
	}
