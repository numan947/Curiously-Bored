...
typedef struct asdf1{
	int msg,st,ed;
	bool operator < (const struct asdf1 & TMP)const
	{
		return st<TMP.st;
	}
...
typedef struct asdf{
	int size,nextByte,rmTotal;
	set<PKT>packets;

	void addPacket(PKT packet)
	{
		//cout<<"ADDING PACKET: "<<packet.msg<<" "<<packet.st<<" "<<packet.ed<<endl;
		packets.insert(packet);
		buffer+=(packet.ed - packet.st + 1);
	}
	void passBuffer()
	{
		if(passedBuffer())
			return;
		int rm = 0;
		set<PKT>::iterator it = packets.begin();

		while(it!=packets.end()){
			if((*it).st == nextByte){
			//	cout<<"REMOVING -> "<<(*it).msg<<" "<<(*it).st<<" "<<(*it).ed<<endl;
				nextByte=(*it).ed+1;
				rm += (*it).ed - (*it).st + 1;
				it = packets.erase(it);
...
the following lines in main function.*/
	...
	int N,M,tmp1,tmp2,tmp3,tmp4,KS=0;
	...
	while(cin>>N>>M){
		...
		do{
			oOrder = 0;
			currentMsg = order[oOrder++];
			buffer = 0;
			int maxBuff = -INF;
			for(int i=0;i<N;i++){
				allmsgs[i].nextByte=1;
				allmsgs[i].rmTotal=0;
				allmsgs[i].packets.clear();
			}

			for(int i=0;i<M;i++){
			//	cout<<"Packet: "<<1+i<<endl;
				PKT currentPkt = allpkts[i];
				int msgId = currentPkt.msg;

				allmsgs[msgId].addPacket(currentPkt);
				if(currentMsg==msgId){
					allmsgs[msgId].passBuffer();

					if(allmsgs[msgId].passedBuffer()){
						for(int tmp = oOrder;tmp<N;tmp++){
							int tmpCur = order[tmp];
							allmsgs[tmpCur].passBuffer();
							if(allmsgs[tmpCur].passedBuffer())
								continue;
							else{
								currentMsg = tmpCur;
								oOrder = tmp+1;
			...
			maxNeedBuff = min(maxBuff, maxNeedBuff);
		}while(next_permutation(order, order+N));

		cout<<"Case "<<++KS<<": "<<maxNeedBuff<<endl<<endl;
