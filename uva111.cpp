...
int correctOrder[MAX];

int lis[MAX];

int studentInput[MAX];
...
/* 			cout<<endl;
			...
			for(int i=1;i<curSize;i++){
				for(int j=i-1;j>=0;j--){
					// cout<<toks[j]-1<<" "<<correctOrder[toks[j-1]]<<" "<<toks[i]-1<<" "<<correctOrder[toks[i]-1]<<endl;
					if(correctOrder[studentInput[j]]<correctOrder[studentInput[i]]){
