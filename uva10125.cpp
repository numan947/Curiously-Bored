...
#define MAX 1001
	...
	vector<pair<ll,pair<int,int> > >cd;





int binary_search1(int i,int j,ll val)
{
	if(i==j)
		return -1;
	int mid = (i+j)>>1;
	if(cd[mid].fi==val)
		return mid;
	if(cd[mid].fi<val)
		return binary_search1(mid+1,j,val);
	else
		return binary_search1(i, mid, val);
...
the following lines in main function.*/
	...
	while(cin>>n){
		...
		for(int i=0;i<n;i++)
			for(int j=i+1;j<n;j++)
					ab.pb(mp(ara[i]+ara[j],mp(i,j)));

		for(int i=0;i<n;i++)
			for(int j=i+1;j<n;j++)
				{
					//cd.pb(mp(ara[i]-ara[j],mp(i,j)));
					cd.pb(mp(ara[j]-ara[i],mp(j,i)));
				}
		...
		for(int i=0;i<ab.size();i++){
			...
			int j=binary_search1(0,cd.size(),ab[i].fi);
			if(j!=-1)
				if(ab[i].fi==cd[j].fi){
					pii a = ab[i].se;
					pii b = cd[j].se;
					if(a.fi!=b.fi&&a.fi!=b.se&&a.se!=b.fi&&a.se!=b.se)
						mx = max(mx,ab[i].fi+ara[cd[j].se.se]);
				}
		}
