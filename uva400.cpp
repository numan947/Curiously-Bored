   ...
   freopen("input.txt", "r", stdin);
	...
	while(cin>>n){
		...
		FOR(i,0,n){
			cin>>inp;
			all.pb(inp);
			longest=max(longest,(int)inp.size());
		}
		int totalCol = 1+max(0,(60-longest)/(longest+2));
		// cout<<totalCol<<endl;

		// cout<<longest<<endl;
		sort(all.begin(),all.end());
		FOR(i,0,all.size()){
			all[i].insert(all[i].end(),longest-all[i].size(),' ');
			
			if(i<all.size()-(all.size()/totalCol))all[i].insert(all[i].end(),2,' ');
		}

		ans.clear();
		int totalRows=ceil(1.0*all.size()/totalCol);
