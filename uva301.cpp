...
void solve(int idx, ll curProfit)
{
	for(int i=0;i<m;i++)
		if(station[i]>n)
			return;
	...
	ll tmpProfit = (orders[idx].end - orders[idx].start)*orders[idx].count;
	for(int i=orders[idx].start;i<orders[idx].end;i++){
		station[i]+=orders[idx].count;
	}
	solve(idx+1,curProfit+tmpProfit);
	for(int i=orders[idx].start;i<orders[idx].end;i++){
		station[i]-=orders[idx].count;
	}
	solve(idx+1,curProfit);
}
...
the following lines in main function.*/
	...
	while(cin>>n>>m>>c){
		...
		FOR(i,0,c)
			cin >> orders[i].start >> orders[i].end >> orders[i].count;
...
This is a correct solution, but TLE


void solve(int curStation, int totalTaken, int totalProfit)
{
	//cout<<endl;
	//db(curStation);
	//db(totalProfit);
	if(curStation==m){ //we're at the destination
	...
	//db(totalTaken);
	for(int i = 0; i<c; i++){
		if(curStation == allOrder[i].end && taken[i])
			totalTaken-=allOrder[i].count;
	}
	//db(totalTaken);

	if(totalTaken == n)
		solve(curStation+1, totalTaken, totalProfit);
	for(int i=0;i<c;i++){
		
		if(taken[i] || curStation!=allOrder[i].start)
			continue;
		
		if(totalTaken+allOrder[i].count <= n){
			taken[i] = 1;
			int tmpProfit = allOrder[i].count * (allOrder[i].end - allOrder[i].start);
			//db("GO");
			solve(curStation,totalTaken+allOrder[i].count,totalProfit+tmpProfit);
			//db("NGO");
			solve(curStation+1,totalTaken+allOrder[i].count,totalProfit+tmpProfit);
