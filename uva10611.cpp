...
the following lines in main function.*/
	...
	while(cin>>N){
		...
		while(Q--){
			...
			auto lh = lower_bound(chimps.begin(),chimps.end(),tmp);
			if(lh==chimps.end())
				left = to_string(chimps[chimps.size()-1]);
			else if(*lh>=tmp){
				//equal, so the one before this is the one we are looking for
				int tg = lh-chimps.begin();
				while(tg>=0 && chimps[tg]>=tmp)
					tg--;
				if(tg>=0)
					left = to_string(chimps[tg]);
				else
					left = "X"; 
			}else{
				//found is greater so, not applicable
				left = "X";
			}

			auto hh = upper_bound(chimps.begin(),chimps.end(),tmp);
