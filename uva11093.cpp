...
int startFrom(int start,int current,int f)
{
	if(start==current)return start;
	else if(f+fuel[current]-need[current]>=0)return startFrom(start,(current+1)%N,f+fuel[current]-need[current]);
	else return current;
}
...
int main()
{
	...
	while(tc--){
		...
		FOR(i,0,N){
			...
			int f = fuel[i]-need[i];
			if(f>=0){
				int p = startFrom(i,(i+1)%N,f);
				if(p==i){ans=i+1;break;}
				else if(p<i)break;
				else p=i;
			}
		}
		if(ans>0)printf("Case %d: Possible from station %d\n",++kase,ans);
