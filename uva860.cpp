...
inline void tokenize(string str,vector<string> &tokens, string delim){ tokens.clear();size_t s = str.find_first_not_of(delim), e=s; while(s!=std::string::npos){e=str.find(delim,s);tokens.push_back(customStrip(str.substr(s,e-s),",.:;!?\"() \n\t"));s=str.find_first_not_of(delim,e);}}
...
//    freopen("output.txt", "w", stdout);
	...
	while(getline(cin,in)){
		if(in=="****END_OF_TEXT****"){
			//process current result + clear containers
			double entropy = 0.0;
			for(map<string,int>::iterator it = myMap.begin();it!=myMap.end();it++){
				entropy += ((it->se)*(log10(lambda) - log10(it->se)))/lambda;
				//cout<<it->fi<<" "<<it->se<<" "<<endl;
			}

			//entropy/=lambda;

			cout<<prec(1)<<lambda<<" "<<entropy<<" "<<(int)round((entropy*100.0/log10(lambda)))<<endl;
			...
			break;
		//transform(toks[i].begin(), toks[i].end(),toks[i].begin(),::lower);
		else{
			toks.clear();
			tokenize(in,toks," ");
			lambda+=toks.size();
			for(int i=0;i<toks.size();i++){
				transform(toks[i].begin(), toks[i].end(),toks[i].begin(),::tolower);
				myMap[toks[i]]++;
			}
		}
