...
bool takenMat[MAX][MAX];
vector<int>edges[MAX];

bool printPath(int cur, int ln, vector<int>vt)
{
	if(ln==0){
		cout<<"("<<1+vt[0];
	...
	{
		int nxtNd = edges[cur][i];
		if(!takenMat[cur][nxtNd] && !vectHas(vt,nxtNd)){
			
			takenMat[cur][nxtNd] = takenMat[nxtNd][cur] = 1;
			vt.pb(nxtNd);

			tmp|=printPath(nxtNd,ln-1,vt);
			
			vt.pop_back();
			takenMat[cur][nxtNd] = takenMat[nxtNd][cur] = 0;
		}
	}
	return tmp;
...
*/
		...
		path.pb(0);
		//nodTaken[0]=1;
		findPath |= printPath(0,ln,path);

		if(!findPath)
			cout<<"no walk of length "<<ln<<endl;
