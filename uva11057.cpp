...
the following lines in main function.*/
	...
	while(cin>>N){
		...
		cin>>M;
		// cout<<M<<endl;
		sort(bookPriceArray,bookPriceArray+N);
		
		int b1=-1,b2=-1,tmp;


		for(int i = N-1;i>=0;i--){
			tmp = bookPriceArray[i];
			int b1Idx = lower_bound(bookPriceArray,bookPriceArray+N,M-tmp)-bookPriceArray;
			if(bookPriceArray[b1Idx]+tmp==M){
				//found match
				if(b1==-1||abs(tmp-bookPriceArray[b1Idx])<abs(b1-b2)){//first match
					b1 = bookPriceArray[b1Idx];
					b2 = tmp;
				}
			}
		}
