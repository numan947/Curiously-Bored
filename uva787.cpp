...
the following lines in main function.*/
	...
	while(cin>>tmp){
		...
		ll mx_ending_here,mn_ending_here,mx_so_far;
		mx_ending_here = mn_ending_here= mx_so_far = ara[0];

		bool zero_found = 0;

		for(int i=1;i<n;i++){
			ll tmpMx = mx_ending_here*ara[i];
			ll tmpMn = mn_ending_here*ara[i];

			mx_ending_here = max(max(tmpMn,tmpMx),ara[i]);
			mn_ending_here = min(min(tmpMn,tmpMx),ara[i]);
