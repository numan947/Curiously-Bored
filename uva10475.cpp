...
void generateRes(int idx, int cnt, int mask)
{
	...
	if(!taken[idx]){
		...
		//cout<<topic[idx]<<"--->";
		vector<int>tmp;
		for(int i=0;i<enemeyList[idx].size();i++){
		//	cout<<" "<<enemeyList[idx][i];
			if(!taken[getID(enemeyList[idx][i])]){
				taken[getID(enemeyList[idx][i])] =1;
				tmp.pb(getID(enemeyList[idx][i]));
			}
		}
		//cout<<endl;
		generateRes(idx+1,cnt+1,mask|(1<<idx));
		for(int i=0;i<tmp.size();i++)
			taken[tmp[i]] = 0;
		taken[idx] = 0;
	}
	generateRes(idx+1,cnt,mask);
...
the following lines in main function.*/
	...
	while(n--){
		...
		FOR(i,0,t){
			cin>>topic[i];
			enemeyList[i].clear();
			transform(topic[i].begin(),topic[i].end(),topic[i].begin(),::toupper);
		}
		sort(topic,topic+t,cmpr);
		FOR(i,0,t){
			getID(topic[i]);
		}
		...
		FOR(i,0,p){
			...
			enemeyList[getID(s1)].pb(s2);
			enemeyList[getID(s2)].pb(s1);
		}
