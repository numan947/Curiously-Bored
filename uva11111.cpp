...
//    freopen("output.txt", "w", stdout);
	...
	while(getline(cin,input)){
		...
		int curSize=0,curVal,flag=1;

		for(int i=0;i<toks.size();i++){
			curVal = stoi(toks[i]);
			if(curSize==0 && curVal<0){ //outer most doll
				curSize = abs(curVal);
				stk.push(curVal);
			}

			else if(curVal<0){
				if(-curVal<curSize){ //enough space
					stk.push(curVal);
					curSize = abs(stk.top());
				}
				else{ //not enough
				...
				if(stk.size()&&stk.top()==-curVal){ //okay nested
					stk.pop();
					if(stk.size())
						curSize = abs(stk.top()) - curVal;
					else curSize = 0;
				}
				else{ //mismatch
					flag=0;
