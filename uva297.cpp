...
int traverse(string s1,string s2,int sz,bool t1, bool t2)
{
	int sum=0;
	if(t1&&t2){
		if(s1[idx1]=='p' && s2[idx2]=='p'){
			FOR(i,0,4){
				idx1++;
				idx2++;
				sum+=traverse(s1,s2,sz/2,1,1);
			}
		}
		else if(s1[idx1]=='p' && s2[idx2]=='f'){
			FOR(i,0,4){
				idx1++;
				//idx2++;
				sum+=traverse(s1,s2,sz/2,1,0);
			}
			sum = max(sum,sz*sz);
		}
		else if(s1[idx1]=='p' && s2[idx2]=='e'){
			FOR(i,0,4){
				idx1++;
				//idx2++;
				sum+=traverse(s1,s2,sz/2,1,0);
		...
		else if(s1[idx1]=='f' && s2[idx2]=='p'){
			FOR(i,0,4){
				//idx1++;
				idx2++;
				sum+=traverse(s1,s2,sz/2,0,1);
			}
			sum = max(sum,sz*sz);
		}
		else if(s1[idx1]=='e' && s2[idx2]=='p'){
			FOR(i,0,4){
				//idx1++;
				idx2++;
				sum+=traverse(s1,s2,sz/2,0,1);
		...
		else if((s1[idx1]=='f' && s2[idx2]=='e')||(s1[idx1]=='f' && s2[idx2]=='f')||(s1[idx1]=='e' && s2[idx2]=='f')){
			sum = sz*sz;
		}
		else if(s1[idx1]=='e' && s2[idx2]=='e'){
			sum=0;
		}
	}
...
the following lines in main function.*/
	...
	while(tc--){
		...
		cout<<"There are "<<traverse(s1,s2,32,1,1)<<" black pixels."<<endl;
