...
bool solve(int idx, int currentVal)
{
	if(idx==n && currentVal==target)
		return true;
	if(idx>=n)
		return false;
	
	if(vs[idx][currentVal+32001])
		return dp[idx][currentVal+32001];
	
	bool ok = false;
	if(idx==0)
		ok|= solve(idx+1,currentVal+ara[idx]);
	else{
		int tmpVal;
		vv.pb(1);
		tmpVal = currentVal+ara[idx];
		if(tmpVal>=-32000 && tmpVal<=32000){
			ok|=solve(idx+1,tmpVal);
		}
		if(ok){
			return ok;
		}
		...
		vv.pb(4);
		tmpVal = currentVal/ara[idx];
		if((currentVal%ara[idx]==0)&& tmpVal>=-32000 && tmpVal<=32000){
			ok|=solve(idx+1,tmpVal);
		}
		if(ok){
			return ok;
	...
	vs[idx][currentVal+32001] = 1;
	return dp[idx][currentVal+32001] = ok;
}
