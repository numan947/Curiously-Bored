...
int T1,T2,n;
vector<int>siteNCPC[MAX];
vector<int>siteBCEW[MAX];

int cnt[MAX];

int dp[MAX][155][155];
int back[MAX][155][155];



int solve(int idx, int remT1, int remT2)
{
	if(idx==n && remT1==0 && remT2==0)
		return 0;
	if(idx==n||remT1<0||remT2<0)
		return INF;
	if(dp[idx][remT1][remT2]!=-1)
		return dp[idx][remT1][remT2];
	int mini = INF;
	int nn,sn;
	for(int i=0;i<=siteNCPC[idx].size();i++){
		int tmp = solve(idx+1,remT1-i,remT2-(siteNCPC[idx].size()-i));
		if(i>0)
			tmp+=siteNCPC[idx][i-1];
		if(i<siteNCPC[idx].size())
			tmp+=siteBCEW[idx][(siteNCPC[idx].size()-i-1)];
		
		if(mini>tmp){
			mini = tmp;
			cnt[idx] = i;
			nn = i;
			sn = siteNCPC[idx].size()-i;
		}
	}

	back[idx][remT1][remT2] = nn;
	return dp[idx][remT1][remT2] = mini;
}
...
the following lines in main function.*/
	...
	while(cin>>T1>>T2){
		...
		int takenFromNCPC = back[idx][remT1][remT2];
		int takenFromBCEW = siteNCPC[idx].size() - takenFromNCPC;
		cout<<takenFromNCPC;
		remT1-=takenFromNCPC;
		remT2-=takenFromBCEW;
		idx++;
		while(idx<n){
			takenFromNCPC = back[idx][remT1][remT2];
			takenFromBCEW = siteNCPC[idx].size() - takenFromNCPC;
			cout<<" "<<takenFromNCPC;
			remT1-=takenFromNCPC;
			remT2-=takenFromBCEW;
			idx++;
