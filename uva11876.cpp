...
void makeSieve(){
	...
	for(int i=2;i*i<=MAX;i++){
		if(sieve[i]){
			//if i is a prime, nullify its multiplications
...
ll countDivisors(ll x)
{
	...
	for(map<int,int>::iterator it=primeDivCount.begin();it!=primeDivCount.end();it++){
...
the following lines in main function.*/
	...
	for(ll i=1;i<=MAX;i++){
		...
		seq[i] = seq[i-1]+countDivisors(seq[i-1]);
	}
	...
	while(T--){
		cin>>A>>B;
		int idx1 = lower_bound(seq,seq+N,A)-seq;
		int idx2 = lower_bound(seq,seq+N,B)-seq;
