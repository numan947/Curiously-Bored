...
pii solve(int remTime)
{
	...
	if(remTime<0)
		return mp(-LINF,-LINF);
	if(remTime<m && remTime<n){
		//beer time
		return mp(0,remTime);
	}

	if(dp[remTime].fi!=-1 &&dp[remTime].se!=-1)
		return dp[remTime];

	pii tmp1 = solve(remTime-m);
	tmp1.fi++;
	pii tmp2 = solve(remTime-n);
	tmp2.fi++;
	return dp[remTime] = maxPLL(tmp1,tmp2);	
}
