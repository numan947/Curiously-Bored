...
typedef struct tt{
	int src,time,duration,target;

	bool operator < (struct tt &TMP)
	{
	...
	void printCurrent()
	{
		cout<<src<<" "<<time<<" "<<duration<<" "<<target<<endl;
	}
}CFS;
...
int findExtention(int t, int e)
{
	...
	for(int i = 0; i<it; i++){
		...
		CFS curExt = cfs[i];
		//curExt.printCurrent();
		if(t >= curExt.time && t <= curExt.time + curExt.duration){
			markMap[curExt.src] = 1;
			int ext = findExtention(t, curExt.target);
			markMap[curExt.src] = 0;
			return ext;
		}
	}
	return e;
...
the following lines in main function.*/
	...
	FOR(iiii,0,N){
		...
		while(cin>>src){
			...
			cfs[it] = CFS{src,tim,len,tgt};
			//revMap[src] = it;
			markMap[src] = markMap[tgt] = 0;
			it++;
		}

		cout<<"SYSTEM "<<iiii+1<<endl;
		int t,e;
		while(cin >> t){
			if(t == 9000)
				break;
			...
			int ans = findExtention(t,e);

			cout<<"AT "<<setw(4)<<setfill('0')<<t<<" CALL TO "<<setw(4)<<setfill('0')<<e<<" RINGS "<<setw(4)<<setfill('0')<<ans<<endl;
		}
	}
