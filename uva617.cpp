...
the following lines in main function.*/
	...
	while(cin>>n){
		...
		// FOR(i,0,vv.size())
		// 	cout<<vv[i].L<<" "<<vv[i].G<<" "<<vv[i].Y<<" "<<vv[i].R<<endl;


		if(!vv.size()){
			cout<<"Case "<<++KS<<": 30-60"<<endl;
			continue;
		}
		...
		for(int speed = 30;speed<=60;speed++){
			...
			for(int i=0;i<vv.size();i++){
				int curMod = vv[i].G+vv[i].Y+vv[i].R;
				double curT = ((3600.0*vv[i].L)/speed);
				//cout<<" "<<fmod(curT,curMod)<<" "<<vv[i].G+vv[i].Y;
				if(GT(fmod(curT,curMod),(vv[i].G+vv[i].Y))){
					can = false;
					break;
				}
			}
