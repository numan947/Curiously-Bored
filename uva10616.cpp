...
int goFindAns(int idx, int rem, ll total)
{
	// cout<<total<<endl;
	if(rem==0 && ((total%D)+D)%D==0)
		return 1;
	else if(rem==0 || idx==N)
		return 0;
	
	if(dp[idx][rem][total]!=-1)
		return dp[idx][rem][total];
	
	dp[idx][rem][total]=goFindAns(idx+1,rem,total)+goFindAns(idx+1,rem-1,(((total+ara[idx])%D)+D)%D);
...
the following lines in main function.*/
	...
	while(cin>>N>>Q){
		...
		FOR(i,0,Q){
			...
			cout<<"QUERY "<<i+1<<": "<<goFindAns(0,M,0)<<endl;
		}
	}
