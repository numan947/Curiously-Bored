...
void apply_lazy(int cur,int l, int r)
{
	if(lazy[cur]!='N'){//there is update,
		if(l==r){
			if(lazy[cur]=='S')
				tree[cur]=1;
			else if(lazy[cur]=='R')
		...
		if(l!=r){
			if(lazy[cur]=='S'){
				...
				if(lazy[left(cur)]=='N')
					lazy[left(cur)] = 'F';
				else if(lazy[left(cur)]=='R')
					lazy[left(cur)]='S';
				else if(lazy[left(cur)]=='S')
					lazy[left(cur)]='R';
				else
					lazy[left(cur)] = 'N';
				...
				if(lazy[right(cur)]=='N')
					lazy[right(cur)] = 'F';
				else if(lazy[right(cur)]=='R')
					lazy[right(cur)]='S';
				else if(lazy[right(cur)]=='S')
					lazy[right(cur)]='R';
				else
					lazy[right(cur)] = 'N';
		...
		lazy[cur]='N';
		//cout<<"Lazy applied"<<endl;
	}
	//cout<<"Lazy not applied"<<endl;
...
void build_tree(int cur,int l, int r)
{
	...
	build_tree(left(cur),l,mid(l,r));
	build_tree(right(cur),1+mid(l,r),r);
...
void update_tree(int cur,int l, int r, int i, int j, char val)
{
	apply_lazy(cur,l,r);
	if(l>r||l>j||r<i)
		return;

	if(l>=i&&r<=j){ //save as lazy
		if(l==r){
			if(val=='S')
				tree[cur]=1;
			else if(val=='R')
				tree[cur]=0;
			else tree[cur]=!tree[cur];
		}
		else{
			if(val=='S'){
				tree[cur]=r-l+1;
	...
	update_tree(left(cur),l,mid(l,r),i,j,val);
	update_tree(right(cur),1+mid(l,r),r,i,j,val);
	tree[cur] = tree[left(cur)]+tree[right(cur)];
}


int query_tree(int cur, int l, int r, int i, int j)
{
	apply_lazy(cur,l,r);
	if(l>r||l>j||r<i)
		return 0;
	...
	int q1 = query_tree(left(cur),l,mid(l,r),i,j);
	int q2 = query_tree(right(cur),1+mid(l,r),r,i,j);
...
the following lines in main function.*/
	...
	while(TC--){
		...
		// update_tree(1,0,n-1,0,17,'S');
		// update_tree(1,0,n-1,0,5,'F');

		// cout<<query_tree(1,0,n-1,1,10)<<endl;
		...
		while(Q--){
			...
			if(ch=='F'){
				...
				update_tree(1,0,n-1,a,b,'R');
			else if(ch=='I')
				update_tree(1,0,n-1,a,b,'F');
			else{
				//cout<<ch<<" "<<a<<" "<<b<<endl;	
				int ans = query_tree(1,0,n-1,a,b);
				printf("Q%d: %d\n",++QQ,ans);
			}
