...
ll getMaxPossible(int pos,int k)
{
	if(k==0||pos<0){
		return 0;
	}
	...
	ll t1 = arr[pos]-arr[pos-m] + getMaxPossible(pos-m,k-1);
	ll t2 = getMaxPossible(pos-1,k);
	dpS[pos][k] = 1;
	return dp[pos][k] = max(t1,t2);
}
...
//	freopen("input.txt", "r", stdin);
	...
	cout<<getMaxPossible(n-1,k)<<endl;
