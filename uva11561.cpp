...
int collectGold(int r, int c)
{
	vis[r][c] = 1;

	if(setHas(spiderSense,mp(r,c)))
		return (grid[r][c]=='G');
	...
	FOR(i,0,4){
		...
		if(newR>=0 &&newR<H && newC>=0 && newC<W && !vis[newR][newC] && (grid[newR][newC]=='.'||grid[newR][newC]=='G'))
			cnt+= collectGold(newR,newC);
	}

	return cnt+ (grid[r][c]=='G');
}
	...
	while(cin>>W>>H){
		...
		FOR(i,0,H){
			FOR(j,0,W){
				...
				if(grid[i][j]=='T'){
					if(j-1>=0)
						spiderSense.insert(mp(i,j-1));
					if(i-1>=0)
						spiderSense.insert(mp(i-1,j));
					if(i+1<H)
						spiderSense.insert(mp(i+1,j));
					if(j+1<W)
						spiderSense.insert(mp(i,j+1));
