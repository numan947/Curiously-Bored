...
bool matchGuess(string tmp, guess g)
{
	...
	for(int i=0;i<4;i++)
		for(int j=0;j<4;j++){
			if(i==j || mrk[j])continue;
			else
				if(tmp[i]==g.s[j] && !mrk[j]){
					n2++;
					mrk[j]=1;
				}
		}
	...
	// cout<<tmp<<" "<<g.s<<endl;
	// cout<<"Target "<<g.n1<<" "<<g.n2<<endl;
	// cout<<"Found "<<n1<<" "<<n2<<endl;
	return n2==g.n2 && n1==g.n1;
}
...
the following lines in main function.*/
	...
	while(t--){
		cin>>g1.s>>g1.n1>>g1.n2;
		cin>>g2.s>>g2.n1>>g2.n2;
		...
		do{
			...
			if(matchGuess(tmp, g1) && matchGuess(tmp, g2))
			{
				ok = true;
				break;
			}
