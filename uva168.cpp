...
vector<char>lightPos;
ll K;

void minatourRun(int mintPos, int thesPos, ll k)
{
	
	bool traped = false;
	while(!traped){
		// cout<<(char)(mintPos+'A')<<endl;
		int goPos = INF;
		for(int i=0;i<AdjList[mintPos].size();i++){
			if(!light[AdjList[mintPos][i]] && AdjList[mintPos][i]!=thesPos){
				goPos = AdjList[mintPos][i];
				break;
			}
		}
		if(k==0){
			light[thesPos] = 1;
			lightPos.pb(thesPos+'A');
			k=K;
		}
		
		if(goPos==INF){
			lightPos.pb(mintPos+'A');
			traped = true;
		}
		else{
			thesPos = mintPos;
...
the following lines in main function.*/
	...
	while(getline(cin,inp)){
		...
		FOR(i,0,m2.size()){
			tokenize(m2[i],m1,":");
			AdjList[m1[0][0]-'A'].clear();
			// sort(m1[1].begin(),m1[1].end());
			for(int j=0;j<m1[1].size();j++)
				AdjList[m1[0][0]-'A'].pb(m1[1][j]-'A');
		}

		minatourRun(curMintPos,curThesPos,K);

		// cout<<lightPos.size()<<endl;
		if(lightPos.size()>1)
		{	
			cout<<lightPos[0];
			FOR(i,1,lightPos.size()-1)
				cout<<" "<<lightPos[i];
			cout<<" ";
		}
		cout<<"/"<<lightPos.back()<<endl;
