	...
	while(SET--){
		...
		while(!done){
			...
			while(!ss.empty()){
				...
				//this station's cargo, unload
				if(ss.top()==cur){
					tmp = ss.top();
					ss.pop();
					tt++;
					done=1;
//					printf("Unloading %d at %d ---> A, time %d\n",tmp,cur,tt);

					continue;
				}
				

				//other station's cargo, queue
				if(qq[cur].size()<Q){
					tmp = ss.top();
					ss.pop();
					qq[cur].push(tmp);
...
//					printf("Unloading %d at %d ---> B, time %d\n",tmp,cur,tt);
				}
				

				//failure, no space in the queue
				else{
					done=0;
//					printf("FAILED TO UNLOAD at %d\n",cur );
					break;
				}
			}
			...
			//loading
			while(!qq[cur].empty()&&ss.size()<S){
				tmp = qq[cur].front();
				qq[cur].pop();
				ss.push(tmp);
				tt++;
				done=0;
//				printf("Loading %d from %d, time %d\n",tmp,cur,tt);
			}
