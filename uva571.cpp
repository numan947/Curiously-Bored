...
bool solve(int ca, int cb)
{
	//cout<<ca<<" "<<cb<<endl;	
	if(ca>A||cb>B||ca<0||cb<0||state[ca][cb])
		return false;
	...
	//pour A to B
	if(ca!=0 && cb<B){
		int toMove = min(B-cb,ca);
		if(solve(ca-toMove,cb+toMove)){
			//cout<<"pour A to B"<<endl;
			//cout<<ca-toMove<<" "<<cb+toMove<<endl;
			result.pb("pour A B");
			return true;
		}
	}
	...
	//pour B to A
	if(cb!=0 && ca<A){
		int toMove = min(A-ca,cb);
		if(solve(ca+toMove,cb-toMove)){
		//	cout<<"pour B to A"<<endl;
		//	cout<<ca+toMove<<" "<<cb-toMove<<endl;
			result.pb("pour B A");
			return true;
		}
