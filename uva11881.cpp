...
double getIRR(double lo, double hi)
{	
	double ans = -100;
	int validCount = 0;
	while(fabs(lo-hi)>0.000000001){
		double mid = (lo+hi)/2;
		// cout<<mid<<" "<<getValue(mid)<<endl;
		// cout<<hi<<" "<<lo<<" "<<mid<<endl;
		// cout<<fabs(hi-lo)<<endl;
		if(LTE(getValue(mid),0)){
			if(EQ(getValue(mid),0)&&!EQ(Roundoff(ans,10000),Roundoff(mid,10000))
...
the following lines in main function.*/
	...
	while(cin>>T){
		...
		if(EQ(ans,-2.00))
			...
			cout<<prec(2)<<Roundoff(ans,100)<<endl;
	}
