...
bool cmpr(Elephant a, Elephant b)
{
	if(a.wt==b.wt)
		return a.iq>b.iq;
	return a.wt<b.wt;
...
the following lines in main function.*/
	...
	while(cin>>elephant[n].wt>>elephant[n].iq){
		elephant[n].id = n;
		lis[n] = 1;
		n++;
	}
	sort(elephant,elephant+n,cmpr);
/* 	FOR(i,0,n){
		cout<<elephant[i].wt<<" "<<elephant[i].iq<<endl;
	} */
	...
	FOR(i,1,n){
		FORd(j,i,0){
			if(elephant[i].wt>elephant[j].wt && elephant[i].iq<elephant[j].iq && lis[i]<lis[j]+1){
				lis[i] = lis[j]+1;
				par[i] = j;
				if(lis[i]>mx){
					mx = lis[i];
