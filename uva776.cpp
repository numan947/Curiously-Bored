...
void putMonkeyInTree(int r, int c, int mnky)
{
	tree[r][c] = mnky;
	...
	for(int i=0;i<8;i++){
		for(int j=0;j<8;j++){
			newR = r+dr[i];
			newC = c+dc[i];
			if(newR>=0&&newR<R&&newC>=0&&newC<C && tree[newR][newC]==-1 && grid[newR][newC]==grid[r][c])
				putMonkeyInTree(newR,newC,mnky);
		}
	}
}
...
the following lines in main function.*/
	...
	while(getline(cin,inp)){
		if(inp=="%")
			...
			FOR(i,0,C){
				FOR(j,0,R){
					colWidth[i] = max(colWidth[i],countDigit(tree[j][i]));
				}
			}
			FOR(i,0,R){
				cout<<setw(colWidth[0])<<right<<tree[i][0];
				FOR(j,1,C){
					cout<<" "<<setw(colWidth[j])<<right<<tree[i][j];
				}
				cout<<endl;
			}
			cout<<"%"<<endl; 
