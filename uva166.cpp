...
int coin[MAX] = {5,10,20,50,100,200};//6
int denm[MAX];
...
int UDP(int val)
{
	...
	for(int i=0;i<6;i++){
		if(val>=coin[i])
			mini = min(mini,1+UDP(val-coin[i]));
	}
	return dp2[val] = mini;
}

int findMinCoinHandChange(int idx, int makeUp)
{
	// cout<<makeUp<<endl;
	if(makeUp==0){
		return 0;
	...
	if(dp[idx][makeUp]!=-1)
		return dp[idx][makeUp];
	
	int cnt = INF;
	// cout<<makeUp<<" "<<coin[idx]<<" "<<denm[idx]<<endl;
	if(makeUp>=coin[idx] && denm[idx]>=1){	
		for(int i=1;i<=denm[idx];i++){
			if(makeUp>=i*coin[idx]){
				cnt = min(cnt, i+findMinCoinHandChange(idx+1,makeUp-i*coin[idx]));
			}else
				break;
		}
	}
	cnt = min(cnt,findMinCoinHandChange(idx+1,makeUp));
	return dp[idx][makeUp] = cnt;
}
...
/* 	FOR(i,0,6)
	...
	cout<<endl;
	ms(dp,-1);
	cout<<findMinCoinHandChange(0,95)<<endl; */
	
	
	
	
	while(cin>>denm[0]>>denm[1]>>denm[2]>>denm[3]>>denm[4]>>denm[5]){
		int ch = 0;
		for(int i=0;i<6;i++)ch+=denm[i];
		if(ch==0)
			break;
		double dd;
		cin>>dd;
		ms(dp,-1);
		for(int i=800;i>=0;i--){
			findMinCoinHandChange(0,i);
		}
		
		int mini = INF;
		int dt = 100.0*(dd+EPS);
		
		for(int i=800;i>=dt;i--){
			
			// cout<<i<<" "<<findMinCoinHandChange(0,i)<<endl;
			// cout<<i-dt<<" "<<UDP(i-dt)<<endl;
			mini = min(mini,UDP(i-dt)+findMinCoinHandChange(0,i));
		}
		cout<<setw(3)<<mini<<endl;  
	}
