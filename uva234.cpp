...
the following lines in main function.*/
	...
	while(cin>>p){
		...
		int minTot;
		int minlev[] = {INF,INF,INF,INF,INF};
		do{
			int tmpLev[5]={0,0,0,0,0};
			// int tmpTotal = 0;
			...
			for(int i=0;i<data.size();i++){
				...
				while(it<v.size()){
					int checkPoint = v[it].fi;
					int lev = v[it].se;
					if((curt)<checkPoint && (curt+data[i])>checkPoint){
						//cout<<"CheckPoint: "<<checkPoint<<"--->"<<curt<<" "<<curt+data[i]<<endl;
						tmpLev[lev]+=min(curt+data[i]-checkPoint,checkPoint-curt);
						it++;

					}
					else if(curt+data[i]==checkPoint){
						it++;
						break;
					}
					else
			...
			//cout<<"TMPTOTAL-->"<<tmpTotal<<endl<<endl;
			
			bool don=true;
			for(int i=0;don&&i<5;i++){
				if(tmpLev[i]==minlev[i])
					continue;
				else if(tmpLev[i]<minlev[i]){
					don=false;
				}
				else
					break;
