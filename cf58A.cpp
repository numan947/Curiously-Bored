...
the following lines in main function.*/
	...
	int i=0;
	while(in[i]!='h' && i<in.size())
		i++;
	h=i;
	
	i++;
	while(in[i]!='e' && i<in.size())
		i++;
	e=i;
	i++;
	while(in[i]!='l' && i<in.size())
		i++;
	l1=i;
	i++;
	while(in[i]!='l' && i<in.size())
	...
	if(h<e&&e<l1&&l1<l2&&l2<o && h<in.size()&&e<in.size()&&l1<in.size()&&l2<in.size()&&o<in.size())
		cout<<"YES"<<endl;
	else
		cout<<"NO"<<endl;
