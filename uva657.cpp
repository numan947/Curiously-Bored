...
int find_dots(int r, int c)
{
	vis[r][c] = 1;
	int newR ,newC,dots=0;
	for(int i=0;i<4;i++){
		newR = r + dr[i];
		newC = c + dc[i];

		if(newR>=0 && newR<h && newC>=0 && newC<w && !vis[newR][newC] && grid[newR][newC]!='.'){
			int tmp = find_dots(newR,newC);
			dots+=tmp;
		}
	}
	if(grid[r][c]=='X')
