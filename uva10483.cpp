...
typedef struct sm{
	double sum,a,b,c;
	bool operator < (const struct sm &tmp)const
	{
		if(EQ(sum,tmp.sum)){
			if(EQ(a,tmp.a)){
				if(EQ(b,tmp.b)){
...
void precalculate()
{
	...
	for(a=0.01;LT(a+a+a,256.00);a+=0.01){
		for(b=a;LT(a+b+b,256.00);b+=0.01){
			double ab = a*b;
			double aPb = a+b;
			...
			//cout<<c<<" "<<ROUNDF(c,100)<<" "<<round(c*100)/100<<endl;
			//a= ROUNDF(a, 100.0);
			//b= ROUNDF(b, 100.0);

			c = round(c*100)/100;

			if(LT(c,b)||GT(a+b+c,256.0))
				continue;

			if(EQ(a+b+c,a*b*c)){
				//cout<<a<<" "<<b<<" "<<c<<": "<<a+b+c<<"------>"<<a*b*c<<endl;
				v.pb({round(100*a*b*c)/100,a,b,c});
			}
		}
	}
...
the following lines in main function.*/
	...
	double sum,l,r,a,b,c;


	double tmp =37.707779;

	//cout<<round(tmp*100.0)/100.0<<endl;
	...
	while(cin>>l>>r){
		...
		{
			if( GTE(v[i].sum,l) && LTE(v[i].sum, r)){
				cout<<prec(2)<<v[i].sum<<" = "<<v[i].a<<" + "<<v[i].b<<" + "<<v[i].c<<" = "<<v[i].a<<" * "<<v[i].b<<" * "<<v[i].c<<endl;
			}
			if(v[i].sum>r)
				break;
		}
...
/*		for(sum=l;LTE(sum,r);sum+=0.01)
			for(a=0.01;LTE(a,sum);a+=0.01){
				for(b=a;LTE(b,sum);b+=0.01){
					c = sum - a - b;
					if(GT(a*b,sum)||LT(c,b)||LT(c,a)||GT(a*b*c,sum))
						break;
					else if(EQ(a*b*c,sum)){
						cout<<prec(2)<<sum<<" = "<<a<<" + "<<b<<" + "<<c<<" = "<<a<<" * "<<b<<" * "<<c<<endl;
						//cout<<flush;
						break;
					}
				}
