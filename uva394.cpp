...
void process(ARA &tmp)
{	
	for(int i=0;i<tmp.dim-1;i++)
		tmp.Cs.pb(-1);
	tmp.Cs.pb(tmp.cd);

	//cout<<tmp.Cs.size()<<endl;
	for(int i = tmp.dim - 2;i>=0;i--){
		int curC = tmp.Cs[i+1]*(tmp.ud[i+1] - tmp.ld[i+1]+1);
		//cout<<curC<<endl;
		tmp.Cs[i]=curC;
	}
	int total = 0;
	for(int i=0;i<tmp.Cs.size();i++){
		total+=(tmp.Cs[i]*tmp.ld[i]);
	}

	//cout<<total<<endl;
	tmp.c0 = tmp.base-total;
...
//    freopen("output.txt", "w", stdout);
	...
	FOR(i,0,N){
		ARA tmp;
		cin>>tmp.name>>tmp.base>>tmp.cd>>tmp.dim;
		FOR(j,0,tmp.dim){
			int x,y;
			cin>>x>>y;
			tmp.ld.pb(x);
