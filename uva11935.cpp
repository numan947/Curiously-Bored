...
the following lines in main function.*/
	...
	while(getline(cin,input)){
		...
		while(true){
			...
			tokenize(input,toks, " ");
			int newPos = stoi(toks[0]);
			double toAdd = 1.0*(newPos-currentPos)*(1.0*fc100/100.0)+leak*(newPos-currentPos);
			currentFuelCons+=toAdd;
			currentPos = newPos;

			// cout<<input<<" "<<currentFuelCons<<" Added--> "<<toAdd<<endl;
			...
			if(toks[1]=="Fuel"){
				...
				maxFuel = max(maxFuel,currentFuelCons);
				currentFuelCons = 0;
			}
			else if(toks[1]=="Mechanic"){
				leak=0;
			}
			else if(toks[1]=="Leak")
				leak++;
