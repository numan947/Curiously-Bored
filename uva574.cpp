...
set<multiset<int, greater<int>>, greater<multiset<int, greater<int>>> >st; 

void solve(int curTotal, int idx, int mask)
{
	if(curTotal>total || idx>n)
		return;
	if(curTotal==total){
		foundAtleastOne = 1;
	...
	solve(curTotal+ara[idx], idx+1,mask|(1<<idx));
	solve(curTotal, idx+1, mask);
}
...
the following lines in main function.*/
	...
	while(cin>>total>>n){
		...
		FORall(it,st){
			multiset<int, greater<int>>tmp = *it;
			multiset<int, greater<int>>::iterator itt = tmp.begin();
			cout<<*itt;
			itt++;
			while(itt!=tmp.end()){
				cout<<"+"<<*itt;
