...
typedef struct tmp{
	string title;
	string author;
	bool operator < (const tmp& b2){
		if(author==b2.author)return title < b2.title;
...
int main()
{
	...
	while(true){
		...
		if(input=="END")break;
		tmp.clear();
		tokenize(input,tmp,"by");
		Book b;
		b.title = strip(tmp[0]);
		b.author = string(tmp[1]);
		mpBook[b.title] = b;
		stock.pb(b);
	}
	...
	while(true){
		...
		string cmd = input.substr(0,6);
		if(cmd!="SHELVE"){
			string book = input.substr(7);
			Book bb = mpBook[book];
			if(cmd=="BORROW"){
				int pos = lower_bound(stock.begin(),stock.end(),bb) - stock.begin();
			...
			if(rets.size()){
				...
				for(int i=0;i<rets.size();i++){
					int pos = lower_bound(stock.begin(),stock.end(),rets[i]) - stock.begin();
					if(pos==0){
						printf("Put %s first\n",rets[i].title.c_str());
						stock.insert(stock.begin()+pos,rets[i]);
					}
					else{
						printf("Put %s after %s\n",rets[i].title.c_str(),stock[pos-1].title.c_str());
						stock.insert(stock.begin()+pos,rets[i]);
					}
				}
				rets.clear();
