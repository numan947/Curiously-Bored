...
double buy(int idx, int rem)
{
	...
	for(int i=1;;i++)
	{
		if(i*offr[idx].fi<=rem)
			mini = min(mini,(offr[idx].se*i)+buy(idx+1,rem-i*offr[idx].fi));
		else{
			mini = min(mini,(offr[idx].se*i)+buy(idx+1,rem-i*offr[idx].fi));
			break;
		}
	...
	mini = min(mini,buy(idx+1,rem));
	vs[idx][rem] = 1;
	return dp[idx][rem] = mini;
}
