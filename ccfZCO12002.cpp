...
typedef struct ct{
	int st,ed;
	bool operator < (const struct ct &TMP)const
	{
		if(st==TMP.st)
			return ed<TMP.ed;
		return st<TMP.st;
...
the following lines in main function.*/
	...
	for(int i=0;i<N;i++){
		...
		if(!(cur.st>=V[0] && cur.ed <=W[Y-1]))
			continue;

		int idx1 = lower_bound(V,V+X,cur.st)-V;
		int idx2 = lower_bound(W,W+Y,cur.ed)-W;
		
		if(V[idx1]!=cur.st)
			idx1--;

		// cout<<cur.st<<" "<<cur.ed<<endl;
		// cout<<V[idx1]<<" "<<W[idx2]<<endl;

		if(V[idx1]>=W[idx2])
			continue;

		mini = min(mini,W[idx2]-V[idx1]+1);
