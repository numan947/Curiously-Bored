...
int solveDP(int idx, int cap)
{
	...
	int t1 = -INF,t2 = -INF;
	if(obj[idx].se<=cap)
		t1 = solveDP(idx-1,cap-obj[idx].se) + obj[idx].fi;
	t2 = solveDP(idx-1,cap);

	dp[idx][cap] = max(t1,t2);
