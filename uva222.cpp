...
double stPosition[MAX], stFuelCost[MAX], mpg, cap, originCost, destDist;
int numStations;
double minCost;

void findBestCost(int position, double fuelLeft, double totalCost)
{
	if(position == numStations){ //reached
		if(GT(minCost,totalCost))
			minCost = totalCost;
		return;
	}

	if(LT(fuelLeft,(stPosition[position+1]-stPosition[position])/mpg)||(GT(fuelLeft, cap/2.0) && LT(fuelLeft,(stPosition[position+1]-stPosition[position])/mpg))){

		double fuelUsed = ((stPosition[position+1]-stPosition[position])/mpg);
		findBestCost(position+1, cap-fuelUsed, 200+totalCost+(cap - fuelLeft)*stFuelCost[position]);
	}
	else{
		if(GT(fuelLeft,cap/2.0))
			findBestCost(position+1,fuelLeft - ((stPosition[position+1]-stPosition[position])/mpg), totalCost);
		else{
			findBestCost(position+1,cap-((stPosition[position+1]-stPosition[position])/mpg), 200+totalCost+(cap - fuelLeft)*stFuelCost[position]);
			findBestCost(position+1,fuelLeft - ((stPosition[position+1]-stPosition[position])/mpg), totalCost);
		}				
	}
...
the following lines in main function.*/
	...
	while(cin>>destDist){
		...
		cin>>cap>>mpg>>originCost>>numStations;
		FOR(i,0,numStations)
			cin>>stPosition[i]>>stFuelCost[i];

		stPosition[numStations] = destDist;
		numStations++;


		minCost = LINF;

		findBestCost(0,cap-(stPosition[0]/mpg),originCost*100.0);

		cout<<"Data Set #"<<++KS<<endl;
		cout<<"minimum cost = $"<<prec(2)<<Roundoff(minCost/100,100)<<endl;
