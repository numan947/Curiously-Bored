...
int verticeEnergy[MAX];
int verticeNeighbourEnergy[MAX];


bool compr(int u, int v)
{
	if(verticeEnergy[u]==verticeEnergy[v])
		return verticeNeighbourEnergy[u]<verticeNeighbourEnergy[v];
	return verticeEnergy[u]>verticeEnergy[v];
}
...
the following lines in main function.*/
	...
	FOR(i,0,m){
		...
		verticeNeighbourEnergy[u]+=verticeEnergy[v];
		verticeNeighbourEnergy[v]+=verticeEnergy[u];
	...
	sort(removeOrder.begin(),removeOrder.end(),compr);
	int totalEnergy = 0;

	FOR(i,0,removeOrder.size()){
		int tmpTotal = 0;
		int u = removeOrder[i];
		for(int j=0;j<AdjList[u].size();j++){
			...
			if(!removed[v])
				tmpTotal+=verticeEnergy[v];
		}
		removed[u] = 1;
		totalEnergy+=tmpTotal;
		// cout<<i+1<<" "<<tmpTotal<<endl;
	}
