...
inline string strip(string s){
	int i = s.size()-1;
	while(i>=0){if(isspace(s[i]))i--;else break;}s.erase(i+1,s.size()-i-1);return s;}
...
// map<char,pii>markMap;

MarkPoint markPoints[150];

void paint(int sr,int sc, char mark)
{
	if(grid[sr][sc]!=' '||vist[sr][sc])
		return;
	vist[sr][sc] = 1;
	bool hasWallSide = true;
	...
	for(int i=0;i<4;i++){
		...
		if(newR>=0&&newR<R && newC>=0&&newC<ColSize[newR] && grid[newR][newC]==' ' && !vist[newR][newC])
			paint(newR,newC,mark);
	}
...
the following lines in main function.*/
	...
	while(getline(cin,inp)){
		...
		FOR(i,0,inp.size()){
			if(inp[i]!=' '&&inp[i]!='_' && contour==' ')
				contour = inp[i];
			if(inp[i]!=contour && inp[i]!=' '){
				markPoints[MC++] = {(char)inp[i],(int)R,(int)i};
				// sr = R;
				// sc = i;
				inp[i]=' ';
			}
		...
		ColSize[R] = inp.size();
		R++;
		C = max((unsigned long)C,inp.size());
		while(getline(cin,inp)){
		...
		FOR(i,0,MC){
			paint(markPoints[i].r,markPoints[i].c, markPoints[i].ch);
		}
