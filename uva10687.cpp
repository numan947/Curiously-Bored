...
double calculateDistance(station s1, station s2)
{
	int dx = s1.X-s2.X;
	int dy = s1.Y-s2.Y;
	dx = dx*dx;
...
the following lines in main function.*/
	...
	while(cin>>N){
		...
		station mn1,mn2;
		double mndst1,mndst2;
		for(int i=0;i<N;i++){
			mndst1 = mndst2 = INF;
			for(int j=0;j<N;j++){
				if(i==j)continue;
				double tmpdst = calculateDistance(sts[i],sts[j]);
				if(GT(mndst1,tmpdst)){
					mndst2 = mndst1;
					mn2 = mn1;
					mn1 = sts[j];
				...
				else if(GT(mndst2,tmpdst)){
					mn2 = sts[j];
					mndst2 = tmpdst;
				}
				else if(EQ(mndst1,tmpdst)){
					if((sts[j].X==mn1.X && sts[j].Y<mn1.Y)||(sts[j].X<mn1.X)){
						mn1 = sts[j];
					}
				}
				else if(EQ(mndst2,tmpdst)){
					if((sts[j].X==mn1.X && sts[j].Y<mn1.Y)||(sts[j].X<mn1.X)){
						mn2 = sts[j];
					}
				}
