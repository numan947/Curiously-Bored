...
bool checkRows(int r1,int r2,int M)
{
	int i=0,j=M-1;
	while(i<M&&j>=0){
		if(grid[r1][i]<0||grid[r2][j]<0)return false;
		if(grid[r1][i]!=grid[r2][j])return false;
		i++;j--;
	}
	return true;
}
...
//    freopen("output.txt", "w", stdout);
	...
	FOR(KASE,0,tc){
		...
		getchar();
		//cout<<M<<endl;
		bool mainFalse=false;
		FOR(i,0,M)FOR(j,0,M){
			cin>>grid[i][j];
			if(grid[i][j]<0)mainFalse=true;
		}
		if(mainFalse){
