...
bool valid(int curR, int curC)
{
	return curR>=0&&curR<R&&curC>=0&&curC<C && grid[curR][curC]!=WATER;
}


void visit(int curR, int curC,int curM, int curN)
{
	grid[curR][curC] = 0;
	int cnt = 0;
	set<pii>bag;
	for(int i=0;i<4;i++){
		int nxtR = curR+dr[i]*curM;
		int nxtC = curC+dc[i]*curN;
		// cout<<nxtR<<" "<<nxtC<<endl;
		if(valid(nxtR,nxtC) && !setHas(bag,mp(nxtR,nxtC))){
			cnt++;
			bag.insert(mp(nxtR,nxtC));
		}
		else
			continue;
		if(grid[nxtR][nxtC]==UNVISITED)
			visit(nxtR,nxtC,curM,curN);
	}	
	swap(curM,curN);
	for(int i=0;i<4;i++){
		int nxtR = curR+dr[i]*curM;
