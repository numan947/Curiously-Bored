...
map<int,int>teamMap;
map<int,list<int>::iterator >teamLastPos;
list<int>qq;

void insertInList(int x)
{
	//cout<<"inseting..."<<x<<endl;
	int t = teamMap[x];


	if(teamLastPos.find(t)!=teamLastPos.end()){
		list<int>::iterator it = teamLastPos[t];
		it++;
		qq.insert(it,x);
		it--;
		teamLastPos[t] = it;
	...
	while(cin>>t){
		...
		while(true){
			...
			if(cmd=="ENQUEUE"){
				...
				int team = teamMap[qq.front()];

				if(teamLastPos[team]==qq.begin())
					teamLastPos.erase(team);
...
*/
				...
				// for(map<int,list<int>::iterator >::iterator it = teamLastPos.begin();it!=teamLastPos.end();it++){
				// 	if(it->se == qq.begin())
				// 		toErase = it->fi;
				// 	else
				// 		--(it->se);
