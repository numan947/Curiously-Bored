...
void findAns(int cur, int total, int currentBag)
{
	if(cur>track)
		return;
	if(total>N)
	...
	if(cur<track && total+lst[cur] <= N){
		//cout<<"Taking  "<<cur<<endl;
		findAns(cur+1,total+lst[cur],currentBag|(1<<cur));
	}
	//cout<<"Skipping  "<<cur<<endl;
	findAns(cur+1,total,currentBag);
}
