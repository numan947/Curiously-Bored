...
pi findBlock(int b,int n){
	for(int i=0;i<n;i++){
		for(int j=0;j<grid[i].len;j++)
			if(grid[i].ara[j]==b)return make_pair(i,j);
	}
}
...
{
	grid[b].ara[grid[b].len]=grid[a].ara[grid[a].len-1];
	grid[b].len++;
	grid[a].len--;
}
...
int main()
{
	...
	while(true){
		...
		if(input=="quit")break;
		helper.clear();
		tokenize(input,helper," ");
		int a = stoi(helper[1]);int b=stoi(helper[3]);
		if(a==b)continue; //ignore
		pi aPos=findBlock(a,n);
		pi bPos=findBlock(b,n);
		if(aPos.fi==bPos.fi)continue;
		if(helper[0]=="move"){
			if(helper[2]=="onto"){
				int x = aPos.fi;
				int y = aPos.se;
				while(true){
					if(grid[x].ara[grid[x].len-1]==a)break;
					moveAtoB(x,grid[x].ara[grid[x].len-1]);
				}
				x=bPos.fi;
				y=bPos.se;
				while(true){
					if(grid[x].ara[grid[x].len-1]==b)break;
					moveAtoB(x,grid[x].ara[grid[x].len-1]);
				}

				moveAtoB(aPos.fi,bPos.fi);
			}
			if(helper[2]=="over"){
				int x = aPos.fi;
				int y = aPos.se;
			...
			if(helper[2]=="onto"){
				...
				int ln=grid[aPos.fi].len;

				for(int i=aPos.se;i<ln;i++){
					grid[bPos.fi].ara[grid[bPos.fi].len]=grid[aPos.fi].ara[i];
					grid[bPos.fi].len++;
					grid[aPos.fi].len--;
				}
			}
