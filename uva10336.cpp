...
void mark(int r, int c)
{
	...
	FOR(i,0,4){
		...
		if(newR>=0&&newR<H&&newC>=0&&newC<W && grid[newR][newC]==grid[r][c] && !vis[newR][newC])
			mark(newR,newC);
	}
}
	...
	FOR(IT,0,TC){
		...
		FOR(i,0,H){
			FOR(j,0,W){
				if(!vis[i][j]){
					mark(i,j);
					countryCounter[grid[i][j]]++;
				}
			}
		}

		cout<<"World #"<<IT+1<<endl;

		FORall(it,countryCounter){
			vv.pb(mp(it->fi,it->se));
		}
