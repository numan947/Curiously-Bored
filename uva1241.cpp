...
//    freopen("output.txt", "w", stdout);
	...
	while(T--){
		...
		while(iter>0){
			...
			int curTotal = pow(2,iter);
			int idx = 0;
			int newIdx = 0;


			while(idx<curTotal){
				if((bb[idx]==1)&&(bb[idx+1]==1)){ //both ones, no match, no winner
					bb.set(newIdx);
				}
				else if(((bb[idx]==1)&&(bb[idx+1]==0))||((bb[idx]==0)&&(bb[idx+1]==1))){ //one is absent, walkover match
					cnt++;
					bb.reset(newIdx);
				}
				else{ //non walkover match
					bb.reset(newIdx);
				}
				newIdx++;
				idx+=2;
