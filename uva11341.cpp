...
int findAnswer(int idx, int hourLeft)
{
	if(hourLeft<0)
		return -INF;
	if(hourLeft==0 && idx<N)
	...
	if(hourLeft==0)
		return 0;
	if(dp[idx][hourLeft]!=-1)
		return dp[idx][hourLeft];
	int mx = 0;
	for(int j=0;j<M;j++){
		if(L[idx][j]>=5)
			mx = max(mx,L[idx][j]+findAnswer(idx+1,hourLeft-(j+1)));
	}

	return dp[idx][hourLeft] = mx;
}
...
the following lines in main function.*/
	...
	while(TC--){
		...
		if(totalHourSpent>M||passed<N){
			cout<<"Peter, you shouldn't have played billiard that much."<<endl;
			continue;
		}
		else{
			ms(dp,-1);
			cout<<"Maximal possible average mark - "<<prec(2)<<Roundoff(1.0*findAnswer(0,M)/N,100)<<"."<<endl;
		}
	}
